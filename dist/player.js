...
`;function M(s,e={}){let{status:a="",dim:t=!1,highlight:o=!1,id:i=""}=e,c=["wl-row"];t&&c.push("dim"),o&&c.push("highlight");let d=a||`<span style="font-size: 11px; color: rgba(10,10,10,0.5);">${s.last}</span>`,p=ze(s.days);return`
  ...
  `}var Ce={id:"collections",tag:"COLLECTIONS \xB7 INTERACTIVE DEMO",coverLabel:"See collections in 30 seconds",closing:{headline:'8 PTPs captured. 3 escalations routed. <span class="grad">Hours back.</span>',sub:"Want Vero to run your morning queue, every morning?"},scenes:[{id:"c-01-worklist",title:"Step 1 \xB7 Worklist",body:`${j} accounts overdue today. Vero works them all at once: calls in <span class="grad">their language</span>, emails in your tone.`,tooltipSide:"left",spotlight:"#run-batch",advanceOn:{click:"#run-batch"},html:`${E}
      ...
      `},{id:"c-02-batch",title:"Step 2 \xB7 Batch running",body:`One queue, ${j} parallel workstreams. Vero follows your <span class="grad">playbook</span> for each: channel, language, escalation rules.`,tooltipSide:"left",spotlight:"#wl-running",html:`${E}
        <style>
...
`,je={id:"deductions",tag:"DEDUCTIONS \xB7 INTERACTIVE DEMO",coverLabel:"See deductions in 30 seconds",closing:{headline:'Short-pay investigated, settlement posted. <span class="grad">Hours saved.</span>',sub:"Want Vero to auto-resolve patterns it's already seen?"},scenes:[{id:"d-01-queue",title:"Step 1 \xB7 Deduction queue",body:`${n.retail.name} paid short by <span class="grad">\u20AC4,230</span> on a \u20AC92K invoice. Vero will pull the evidence.`,tooltipSide:"left",spotlight:"#ded-investigate",advanceOn:{click:"#ded-investigate"},html:`${F}
      ...
      `},{id:"d-02-evidence",title:"Step 2 \xB7 Evidence chain",body:'Vero pulls the invoice, purchase order, proof of delivery, contract, and any active <span class="grad">trade-promo agreement</span>, then links them automatically.',tooltipSide:"bottom",spotlight:"#evidence-chain",html:`${F}
        <style>
      ...
      `},{id:"d-03-recommend",title:"Step 3 \xB7 Vero's plan",body:'Evidence assembled, plan drafted. <span class="grad">One click</span> to apply.',tooltipSide:"left",spotlight:"#apply-btn",advanceOn:{click:"#apply-btn"},html:`${F}
        <style>
      ...
      `},{id:"d-04-done",title:"Step 4 \xB7 Posted",body:'Settled in under 5 minutes. Vero spotted <span class="grad">14 similar cases</span> this quarter. Want Vero to resolve those automatically?',tooltipSide:"right",spotlight:"#ded-vero-card",advanceOn:{click:"#ded-yes"},html:`${F}
        <style>
...
`;function ie({weeks:s,highlight:e=-1,resolved:a=!1}){let g=y=>36+y/(s.length-1)*668,h=Math.max(...s)*1.1,f=Math.min(...s)*.85,m=y=>18+(1-(y-f)/(h-f))*174,l=3,u=s.slice(0,l).map((y,w)=>`${w===0?"M":"L"}${g(w).toFixed(1)},${m(y).toFixed(1)}`).join(" "),A=s.slice(l-1).map((y,w)=>`${w===0?"M":"L"}${g(w+l-1).toFixed(1)},${m(y).toFixed(1)}`).join(" "),O=s.slice(l-1).map((y,w)=>`${g(w+l-1).toFixed(1)},${m(y*1.1).toFixed(1)}`),G=s.slice(l-1).map((y,w)=>`${g(w+l-1).toFixed(1)},${m(y*.92).toFixed(1)}`).reverse(),N=`M${O.join(" L")} L${G.join(" L")} Z`,K=[0,3,6,9,12].map(y=>`<text x="${g(y)}" y="212" font-size="9" fill="rgba(10,10,10,0.45)" text-anchor="middle">W${y+1}</text>`).join(""),k=[0,.33,.66,1].map(y=>{let w=18+y*174;return`<line x1="36" y1="${w}" x2="704" y2="${w}" stroke="rgba(10,10,10,0.05)" stroke-width="1"/>`}).join(""),_=[h,h*.66+f*.34,h*.33+f*.67,f].map((y,w)=>`<text x="30" y="${18+w/3*174+3}" font-size="9" fill="rgba(10,10,10,0.4)" text-anchor="end">\u20AC${(y/1e3).toFixed(0)}K</text>`).join(""),V=e>=0?`
  ...
  `}function Y(s,e){let a=Ee[s],t=e||a.defaultIntensity,o=a.weeks(t),i=a.risk(t);return`
    <div class="sc-head">
      <div>
...
`,Ie={id:"vero-chat",tag:"VERO CHAT \xB7 INTERACTIVE DEMO",coverLabel:"See Vero Chat in 30 seconds",closing:{headline:'Ask anything. Get answers and <span class="grad">actions</span>.',sub:"Want Vero to monitor your risk landscape and brief you every morning?"},scenes:[{id:"v-01-ask",title:"Step 1 \xB7 Ask",body:'Type any question. Vero pulls from your <span class="grad">live data</span>. No SQL, no joins, no waiting on BI.',tooltipSide:"left",spotlight:"#send-btn",advanceOn:{click:"#send-btn"},html:`${H}
      ...
      `},{id:"v-02-thinking",title:"Step 2 \xB7 Thinking",body:'Vero calls the right tools (aging report, credit scoring, communications history) <span class="grad">in parallel</span>.',tooltipSide:"right",spotlight:"#tool-stack",html:`${H}
        <style>
      ...
      `},{id:"v-03-answer",title:"Step 3 \xB7 Answer",body:'Ranked, scored, with the why. The action chips are <span class="grad">one click</span>. Not just an answer, an offer to act.',tooltipSide:"top",spotlight:"#schedule-calls-btn",advanceOn:{click:"#schedule-calls-btn"},html:`${H}
        <style>
      ...
      `},{id:"v-04-done",title:"Step 4 \xB7 Done",body:'Three calls scheduled in <span class="grad">one click</span>. Want Vero to flag risks and brief you every morning?',tooltipSide:"top",spotlight:"#chat-yes",advanceOn:{click:"#chat-yes"},html:`${H}
        <style>
