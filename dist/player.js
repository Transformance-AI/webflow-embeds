...
`;var Y={};function $(s){Y[s.id]=s,document.querySelectorAll(`transformance-tour[data-tour="${s.id}"]`).forEach(e=>{e._mounted||e._tryMount()})}var K=class extends HTMLElement{constructor(){super(),this._mounted=!1,this._stepIndex=0,this._tour=null}connectedCallback(){this._mounted||this._tryMount()}_tryMount(){let e=this.dataset.tour,a=e&&Y[e];a&&(this._mounted=!0,this._tour=a,this._shadow=this.attachShadow({mode:"open"}),this._shadow.innerHTML=`
      <style>${U}</style>
...
`,ve={id:"cash-app",tag:"CASH APPLICATION \xB7 INTERACTIVE DEMO",coverLabel:"See cash application in 30 seconds",closing:{headline:`That's 90% of cash application, <span class="grad">automated</span>.`,sub:"Want to see it on your own data?"},scenes:[{id:"01-inbox",title:"Step 1 \xB7 Inbox",body:"Three remittances arrived overnight. Vero already started on the first.",tooltipSide:"right",spotlight:"#row-hero",advanceOn:{click:"#row-hero"},html:`${S}
      ...
      `}]};$(ve);function ge(s){return s>=36?"red":s>=16?"amber":"green"}var E=[{co:i.energy,balance:"\u20AC179K",days:45,last:"Last Fri"},{co:i.logistics,balance:"\u20AC92K",days:18,last:"Never"},{co:i.pharma,balance:"\u20AC47K",days:22,last:"Email sent"},{co:i.retail,balance:"\u20AC18K",days:60,last:"3d ago"},{co:i.tech,balance:"\u20AC22K",days:35,last:"Call Thu"},{co:i.food,balance:"\u20AC14K",days:28,last:"1w ago"},{co:i.motors,balance:"\u20AC63K",days:12,last:"Never"},{co:i.build,balance:"\u20AC38K",days:40,last:"Email sent"}],he="\u20AC475K",j=E.length,M=`
  <style>
    /* Base scene frame */
...
`;function T(s,e={}){let{status:a="",dim:t=!1,highlight:r=!1,id:n=""}=e,c=["wl-row"];t&&c.push("dim"),r&&c.push("highlight");let d=a||`<span style="font-size: 11px; color: rgba(10,10,10,0.5);">${s.last}</span>`,p=ge(s.days);return`
    <div class="${c.join(" ")}"${n?` id="${n}"`:""}>
      ${u(s.co)}
      <div>
  ...
  `}var be={id:"collections",tag:"COLLECTIONS \xB7 INTERACTIVE DEMO",coverLabel:"See collections in 30 seconds",closing:{headline:'8 PTPs captured. 3 escalations routed. <span class="grad">Hours back.</span>',sub:"Want Vero to run your morning queue, every morning?"},scenes:[{id:"c-01-worklist",title:"Step 1 \xB7 Worklist",body:`${j} accounts overdue today. Vero works them all at once: calls in <span class="grad">their language</span>, emails in your tone.`,tooltipSide:"left",spotlight:"#run-batch",advanceOn:{click:"#run-batch"},html:`${M}
        <div class="head">
          <div>
            ...
            <div class="head-sub">${j} accounts \xB7 ${he} past due \xB7 updated 2 min ago</div>
          </div>
...
`;function J({weeks:s,highlight:e=-1,resolved:a=!1}){let h=m=>36+m/(s.length-1)*668,y=Math.max(...s)*1.1,b=Math.min(...s)*.85,f=m=>18+(1-(m-b)/(y-b))*174,l=3,g=s.slice(0,l).map((m,w)=>`${w===0?"M":"L"}${h(w).toFixed(1)},${f(m).toFixed(1)}`).join(" "),C=s.slice(l-1).map((m,w)=>`${w===0?"M":"L"}${h(w+l-1).toFixed(1)},${f(m).toFixed(1)}`).join(" "),L=s.slice(l-1).map((m,w)=>`${h(w+l-1).toFixed(1)},${f(m*1.1).toFixed(1)}`),D=s.slice(l-1).map((m,w)=>`${h(w+l-1).toFixed(1)},${f(m*.92).toFixed(1)}`).reverse(),B=`M${L.join(" L")} L${D.join(" L")} Z`,q=[0,3,6,9,12].map(m=>`<text x="${h(m)}" y="212" font-size="9" fill="rgba(10,10,10,0.45)" text-anchor="middle">W${m+1}</text>`).join(""),k=[0,.33,.66,1].map(m=>{let w=18+m*174;return`<line x1="36" y1="${w}" x2="704" y2="${w}" stroke="rgba(10,10,10,0.05)" stroke-width="1"/>`}).join(""),_=[y,y*.66+b*.34,y*.33+b*.67,b].map((m,w)=>`<text x="30" y="${18+w/3*174+3}" font-size="9" fill="rgba(10,10,10,0.4)" text-anchor="end">\u20AC${(m/1e3).toFixed(0)}K</text>`).join(""),I=e>=0?`
    <line x1="${h(e)}" y1="18" x2="${h(e)}" y2="192" stroke="${a?"#019273":"#ef4444"}" stroke-width="1.5" stroke-dasharray="3 3" opacity="0.5"/>
    <circle cx="${h(e)}" cy="${f(s[e])}" r="6" fill="${a?"#019273":"#ef4444"}"/>
    <circle cx="${h(e)}" cy="${f(s[e])}" r="11" fill="none" stroke="${a?"#019273":"#ef4444"}" stroke-width="1.5" opacity="0.4"/>
    <text x="${h(e)}" y="${f(s[e])-16}" font-size="10" font-weight="500" fill="${a?"#019273":"#ef4444"}" text-anchor="middle">\u20AC${(s[e]/1e3).toFixed(0)}K</text>
  `:"";return`
  ...
  `}function F(s,e){let a=me[s],t=e||a.defaultIntensity,r=a.weeks(t),n=a.risk(t);return`
    <div class="sc-head">
      <div>
  ...
  `}var ke={id:"predictions",tag:"CASH FLOW FORECASTING \xB7 INTERACTIVE DEMO",coverLabel:"See cash predictions in 30 seconds",closing:{headline:'A \u20AC170K dip avoided. Forecast back on track. <span class="grad">Hours back.</span>',sub:"Want Vero to monitor and act on signals every morning?"},scenes:[{id:"p-01-forecast",title:"Step 1 \xB7 13-week forecast",body:'Cash holding strong, except a <span class="grad">\u20AC170K dip</span> in week 6. Vero spotted the risk before it lands.',tooltipSide:"left",spotlight:"#dip-investigate",advanceOn:{click:"#dip-investigate"},html:`${P}
      ...
      `},{id:"p-03-atrisk",title:"Step 3 \xB7 At-risk invoices",body:`${i.energy.name}'s \u20AC179K is the biggest exposure. <span class="grad">88% likely to slip</span> based on past behaviour plus signals.`,tooltipSide:"bottom",spotlight:"#energy-row",advanceOn:{click:"#energy-row"},html:`${P}
        ...
        <div class="card">
          ...
          ${fe.map((s,e)=>{let a=e===0,t=s.risk>=80?"red":s.risk>=65?"amber":"gray",r=s.risk>=80?"#ef4444":s.risk>=65?"#f59e0b":"#94a3b8";return`
      ...
      `},{id:"p-05-resolved",title:"Step 5 \xB7 Forecast restored",body:'Dip mitigated. Forecast back on trend. Want Vero to <span class="grad">monitor and act</span> on these signals every morning?',tooltipSide:"top",spotlight:"#pred-yes",advanceOn:{click:"#pred-yes"},html:`${P}
        ...
        <div class="chart-card">
          ...
          ${J({weeks:ue,highlight:6,resolved:!0})}
        </div>
              ...
              <div class="msg-vero-content">
                <div class="msg-vero-bubble">
                  ...
                  <div class="ranked-list">
                    ${we.slice(0,3).map((s,e)=>{let a=s.risk>=80?"r-red":s.risk>=65?"r-amber":"r-gray";return`
...
`,ze={id:"vero-chat-v2",tag:"VERO AGENT \xB7 INTERACTIVE DEMO",coverLabel:"Meet Vero, your AR team lead",closing:{headline:'Reviewed, approved, fired. <span class="grad">One agent</span> running your AR.',sub:"This is what your mornings could look like."},scenes:[{id:"vb-01-recap",title:"Step 1 \xB7 Overnight recap",body:`Vero already handled the obvious work. Below is what's left, <span class="grad">waiting on you</span>.`,tooltipSide:"top",spotlight:"#vb-start-review",advanceOn:{click:"#vb-start-review"},html:`${O}
      ...
      `},{id:"vb-02-jes",title:"Step 2 \xB7 Cash app \xB7 2 JEs",body:'Every ledger line is visible. No black box, <span class="grad">no surprises</span>.',tooltipSide:"top",spotlight:"#vb-approve-jes",advanceOn:{click:"#vb-approve-jes"},html:`${O}
        <style>
      ...
      `},{id:"vb-03-calls",title:"Step 3 \xB7 Collections \xB7 3 calls",body:'Per-customer script, language and time. Vero waits for your <span class="grad">green light</span>.',tooltipSide:"top",spotlight:"#vb-approve-calls",advanceOn:{click:"#vb-approve-calls"},html:`${O}
        <style>
      ...
      `},{id:"vb-04-flag",title:"Step 4 \xB7 Deductions \xB7 1 flag",body:'Vero settled 5 itself. This one hit the <span class="grad">guardrail</span>, packaged for you.',tooltipSide:"top",spotlight:"#vb-send-to-queue",advanceOn:{click:"#vb-send-to-queue"},html:`${O}
        <style>
      ...
      `},{id:"vb-05-schedule",title:"Step 5 \xB7 Schedule as a task",body:'Six items in three clicks. Let Vero run this morning <span class="grad">every morning</span>.',tooltipSide:"top",spotlight:"#vb-schedule",advanceOn:{click:"#vb-schedule"},html:`${O}
        <style>
...
`;var X=[{main:"Your AR, on autopilot.",sub:"From chaos to clarity in seconds."},{main:"One agent. Every workflow.",sub:"Collections, payments, deductions, forecasts."},{main:"You ask. Vero delivers.",sub:"The AI finance agent that gets things done."}],_e=["\u20AC890K cleared, 3 JEs posted","PTP \u20AC179K confirmed for 25 Apr","\u20AC3,780 valid trade promo, \u20AC450 to review","\u20AC14.2M projected, +6% from new PTP"],Se=["Ask","Ingest","Analyze","Execute","Complete","Review","Report"],je=[3,7,5,9,4,8,6,10,3,7,5,8,4,6,9,5,7,3],ee=[{x1:195,y1:55,x2:370,y2:215},{x1:185,y1:170,x2:370,y2:235},{x1:190,y1:305,x2:370,y2:260},{x1:185,y1:420,x2:370,y2:280}],se=[{x1:530,y1:215,x2:660,y2:50},{x1:530,y1:235,x2:660,y2:165},{x1:530,y1:260,x2:660,y2:300},{x1:530,y1:280,x2:660,y2:405}],ae={x1:780,y1:225,x2:730,y2:400},Ce=[{p:0,d:0},{p:1,d:2200},{p:2,d:4e3},{p:3,d:5800},{p:4,d:7800},{p:5,d:9800},{p:6,d:13e3},{p:7,d:16500},{p:-1,d:2e4}],Ae=`
...
`,G=class extends HTMLElement{constructor(){super(),this._phase=-1,this._cycle=0,this._timers=[],this._avatarTimers=[],this._started=!1,this._paused=!1,this._observer=null}connectedCallback(){if(this._shadow)return;this._shadow=this.attachShadow({mode:"open"}),this._shadow.innerHTML=`<style>${Z}</style>${Ae}`,this._q=t=>this._shadow.querySelector(t),this._buildStaticBits();let e=()=>{this._started||(this._started=!0,this._runCycle())},a=()=>{"requestIdleCallback"in window?window.requestIdleCallback(e,{timeout:500}):setTimeout(e,200)};"IntersectionObserver"in window?(this._observer=new IntersectionObserver(t=>{t[0].isIntersecting?(this._paused=!1,this._started||a()):this._paused=!0},{threshold:.01}),this._observer.observe(this)):a()}disconnectedCallback(){this._timers.forEach(clearTimeout),this._timers=[],this._clearAvatarTimers(),this._observer?.disconnect()}_buildStaticBits(){let e=this._q("#waveform");je.forEach((n,c)=>{let d=document.createElement("div");d.className="waveform-bar",d.setAttribute("data-h",n),d.style.transitionDelay=c*30+"ms",e.appendChild(d)});let a=this._q("#progress-steps");Se.forEach((n,c)=>{let d=document.createElement("div");d.className="progress-step",d.innerHTML=`<div class="progress-dot" data-step="${c}"></div><span class="progress-label" data-step="${c}">${n}</span>`,a.appendChild(d)});let t=n=>{let c=(n.x1+n.x2)/2,d=(n.x1+n.x2)/2;return`M${n.x1},${n.y1} C${c},${n.y1} ${d},${n.y2} ${n.x2},${n.y2}`},r=n=>Math.sqrt(Math.pow(n.x2-n.x1,2)+Math.pow(n.y2-n.y1,2))*1.5;this._allConns=[];for(let n=0;n<4;n++){let c=this._q(`#conn-in-${n}`),d=t(ee[n]),p=r(ee[n]);c.setAttribute("d",d),c.style.strokeDasharray=p,c.style.strokeDashoffset=p,c.style.transition=`stroke-dashoffset 0.8s cubic-bezier(0.22,1,0.36,1) ${n*.12}s`,this._allConns.push({el:c,len:p,phase:2})}for(let n=0;n<4;n++){let c=this._q(`#conn-out-${n}`),d=t(se[n]),p=r(se[n]);c.setAttribute("d",d),c.style.strokeDasharray=p,c.style.strokeDashoffset=p,c.style.transition=`stroke-dashoffset 0.8s cubic-bezier(0.22,1,0.36,1) ${n*.12}s`,this._allConns.push({el:c,len:p,phase:3})}{let n=this._q("#conn-cross"),c=t(ae),d=r(ae);n.setAttribute("d",c),n.style.strokeDasharray=d,n.style.strokeDashoffset=d,n.style.transition="stroke-dashoffset 0.8s cubic-bezier(0.22,1,0.36,1) 0.3s",this._allConns.push({el:n,len:d,phase:4})}this._drawMiniChart(!1)}_drawMiniChart(e){let a=[6,5.5,6.2,5.8,6.5,7,e?7.8:7.2],t=130,r=40,n=4,c=8.5,d=4.5,p=b=>n+b/(a.length-1)*(t-2*n),v=b=>n+(c-b)/(c-d)*(r-2*n),x=a.map((b,f)=>(f===0?"M":"L")+p(f)+","+v(b)).join(" "),h=this._q("#mini-chart"),y=`<path d="${x}" fill="none" stroke="${e?"#6366f1":"#d1d5db"}" stroke-width="2" stroke-linecap="round" style="transition:all 0.8s ease"/>`;e&&(y+=`<path d="M${p(5)},${v(7)} L${p(6)},${v(7.8)} L${p(6)},${v(7.2)} L${p(5)},${v(7)} Z" fill="#818cf8" opacity="0.2"/>`,y+=`<circle cx="${p(6)}" cy="${v(7.8)}" r="3" fill="#6366f1"/>`,y+=`<line x1="${p(5)}" y1="${v(7.2)}" x2="${p(6)}" y2="${v(7.2)}" stroke="#d1d5db" stroke-width="1" stroke-dasharray="3 2"/>`),h.innerHTML=y}_clearAvatarTimers(){this._avatarTimers.forEach(e=>{clearTimeout(e),clearInterval(e)}),this._avatarTimers=[]}_animateVeroAvatar(e,a){if(!e)return;let t=e.querySelector(".vero-avatar-text");if(!t)return;let r=t.querySelectorAll("span");if(a==="idle"){t.style.color="#10B981",r[0].textContent="0",r[1].textContent="0";let n=()=>{r[0].textContent="-",this._avatarTimers.push(setTimeout(()=>{r[0].textContent="0"},150)),this._avatarTimers.push(setTimeout(n,700+Math.random()*2e3))};this._avatarTimers.push(setTimeout(n,700+Math.random()*1e3));let c=()=>{r[1].textContent="-",this._avatarTimers.push(setTimeout(()=>{r[1].textContent="0"},150)),this._avatarTimers.push(setTimeout(c,1e3+Math.random()*2500))};this._avatarTimers.push(setTimeout(c,1e3+Math.random()*1500))}else if(a==="thinking"){t.style.color="#6EE7B7";let n=setInterval(()=>{r[0].textContent=Math.random()>.5?"1":"0",r[1].textContent=Math.random()>.5?"1":"0"},80);this._avatarTimers.push(n)}else a==="done"&&(t.style.color="#10B981",r[0].textContent="^",r[1].textContent="^")}_setPhase(e){this._phase=e,this._clearAvatarTimers();let a=e===0||e===6,t=e>=1&&e<=4,r=e===5,n=e===7;this._q("#chat-overlay").classList.toggle("visible",a);let d=this._q("#chat-human");d.className="chat-human"+(e===0?" visible":e===6?" faded":""),d.style.transitionDelay=e===0?"0.3s":"0s";let p=this._q("#chat-thinking");p.className="chat-vero-thinking"+(e===0?" visible":""),p.style.transitionDelay=e===0?"1s":"0s";let v=this._q("#chat-summary");v.className="chat-vero-summary"+(e===6?" visible":""),v.style.transitionDelay=e===6?"0.5s":"0s",e===0&&this._animateVeroAvatar(this._q("#vero-avatar-chat-thinking"),"thinking"),e===6&&this._animateVeroAvatar(this._q("#vero-avatar-chat-summary"),"done"),this._q("#canvas-layer").classList.toggle("visible",t);for(let l=0;l<4;l++)this._q("#input-"+l).classList.toggle("visible",e>=1);this._allConns.forEach(l=>{l.el.style.strokeDashoffset=e>=l.phase?"0":l.len});let x=this._q("#vero-hub");x.classList.toggle("visible",e>=2),x.classList.toggle("pulsing",e>=2&&e<=4),this._q("#vero-hub-card").classList.toggle("active",e>=3);let h=this._q("#vero-hub-status");h.textContent=e<3?"Analyzing...":"Running 4 workflows",h.style.color=e>=3?"var(--violet-600)":"var(--gray-400)",e>=2&&this._animateVeroAvatar(this._q("#vero-avatar-hub"),e>=3?"thinking":"idle");for(let l=0;l<4;l++){let g=this._q("#output-"+l);g.classList.toggle("visible",e>=3),g.classList.toggle("done",e>=4),this._q("#check-"+l).classList.toggle("visible",e>=4)}for(let l=0;l<3;l++)this._q("#inv-"+l).classList.toggle("done",e>=4),this._q("#badge-"+l).classList.toggle("done",e>=4);this._q("#cleared-text").classList.toggle("visible",e>=4),this._q("#waveform").querySelectorAll(".waveform-bar").forEach(l=>{let g=e>=3;l.classList.toggle("active",g);let C=parseFloat(l.getAttribute("data-h")),L=g?Math.min(1,C*2.2/22):.14;l.style.transform=`scaleY(${L})`}),this._q("#call-detail").classList.toggle("visible",e>=4);for(let l=0;l<3;l++){let g=this._q("#ev-check-"+l);g.classList.toggle("done",e>=4),g.classList.toggle("pending",e<4),g.style.transitionDelay=l*150+400+"ms"}if(this._q("#verdict-fill").classList.toggle("done",e>=4),this._drawMiniChart(e>=4),this._q("#forecast-uplift").classList.toggle("visible",e>=4),this._q("#checklist-overlay").classList.toggle("visible",r),r){this._animateVeroAvatar(this._q("#vero-avatar-checklist"),"thinking"),this._resetTodos();for(let l=0;l<4;l++)(g=>{this._timers.push(setTimeout(()=>this._checkTodo(g),600+g*500))})(l)}if(this._q("#tagline-overlay").classList.toggle("visible",n),n){let l=X[this._cycle%X.length];this._q("#tagline-content").innerHTML=`<div class="tagline-main">${l.main}</div><div class="tagline-sub">${l.sub}</div>`}let f=Math.min(100,e/7*100);this._q("#progress-fill").style.width=f+"%",this._q("#progress-steps").querySelectorAll(".progress-dot").forEach(l=>{let g=parseInt(l.getAttribute("data-step"));l.className="progress-dot"+(e>=g?g<=1?" active-v":g<=4?" active-e":" active-b":"")}),this._q("#progress-steps").querySelectorAll(".progress-label").forEach(l=>{let g=parseInt(l.getAttribute("data-step"));l.classList.toggle("active",e>=g)})}_resetTodos(){for(let e=0;e<4;e++)this._q("#todo-box-"+e).className="check-box",this._q("#todo-box-"+e).innerHTML="",this._q("#todo-label-"+e).className="check-label",this._q("#todo-sub-"+e).className="check-sub",this._q("#todo-sub-"+e).textContent="Processing...";this._q("#checklist-footer").classList.remove("visible")}_checkTodo(e){let a=this._q("#todo-box-"+e);a.className="check-box checked",a.innerHTML='<svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M2.5 6L5 8.5L9.5 3.5" stroke="white" stroke-width="1.5" stroke-linecap="round"/></svg>',this._q("#todo-label-"+e).classList.add("checked");let t=this._q("#todo-sub-"+e);t.classList.add("checked"),t.textContent=_e[e],e===3&&this._q("#checklist-footer").classList.add("visible")}_runCycle(){this._timers.forEach(clearTimeout),this._timers=[],Ce.forEach(e=>{this._timers.push(setTimeout(()=>{this._paused||(e.p===-1?(this._cycle++,this._runCycle()):this._setPhase(e.p))},e.d))})}};customElements.get("transformance-hero")||customElements.define("transformance-hero",G);var te=`
:host{
  --tf-canvas:#f6f4f1;
  --tf-dark:#000;
  --tf-ink:#000;
...
`;var ie={"cash-app":{eyebrow:"PRODUCT \xB7 CLEARMATCH",headline:"Cash application that matches payments to invoices, <grad>without templates.</grad>",claims:[{icon:"sparkles",text:"99.7% extraction accuracy out of the box"},{icon:"chart-up",text:"85-95% straight-through to GL within 90 days"},{icon:"layers",text:"Vision LLMs read remittances natively"},{icon:"check-circle",text:"Deploy in 4-8 weeks, not 4-8 months"}],ctaText:"See ClearMatch",href:"/solutions/cash-application",imageUrl:"https://cdn.prod.website-files.com/684931abb239b84984296d93/69df9ed7814b30f551626c60_e57293d5e53a3981daeaa229a2ec9fad_card-cash-app.avif",imageAlt:"ClearMatch cash application dashboard"},collections:{eyebrow:"PRODUCT \xB7 COLLECTPULSE",headline:"Collections agents that <grad>actually call</grad> \u2014 in 70+ languages.",claims:[{icon:"phone",text:"Autonomous AI calling agent, 70+ languages"},{icon:"globe",text:"Multilingual dunning across regions"},{icon:"chart-up",text:"DSO reduction without growing the team"},{icon:"brain",text:"Vero remembers every customer interaction"}],ctaText:"See CollectPulse",href:"/solutions/collections",imageUrl:"https://cdn.prod.website-files.com/684931abb239b84984296d93/69df9effac59f55ed2162c5c_89904a67e2dbf13e9bdb59df36357cc2_card-collections.avif",imageAlt:"CollectPulse collections dashboard"},deductions:{eyebrow:"PRODUCT \xB7 CLAIMIQ",headline:"Deductions resolution from <grad>investigation to recovery.</grad>",claims:[{icon:"receipt",text:"AI-driven claims investigation, no rules to maintain"},{icon:"sparkles",text:"Trade promotion, chargebacks, and disputes in one queue"},{icon:"chart-up",text:"Recovery rates from 50-60% manual \u2192 80%+ automated"},{icon:"check-circle",text:"Built for CPG-scale deduction volumes"}],ctaText:"See ClaimIQ",href:"/solutions/deductions",imageUrl:"https://cdn.prod.website-files.com/684931abb239b84984296d93/69df9f1d00e7328a348be73d_641b78d95aecfce017fced10a3f3b5e4_card-ded-dash.avif",imageAlt:"ClaimIQ deductions dashboard"},"cash-forecast":{eyebrow:"PRODUCT \xB7 CASHPULSE",headline:"Cash forecasting <grad>built from your AR data,</grad> not historical bank balances.",claims:[{icon:"wallet",text:"Invoice-level prediction, not aging-bucket averages"},{icon:"sparkles",text:"AI learns each customer's payment patterns"},{icon:"clock",text:"Live runway and tightest week, updated in real time"},{icon:"chart-up",text:"13-week rolling forecast that finance teams actually trust"}],ctaText:"See CashPulse",href:"/solutions/cash-flow-forecasting",imageUrl:"https://cdn.prod.website-files.com/684931abb239b84984296d93/69df9feacd0cbc06fa2894ef_1070a6ecf388ba7555d1f167ef194969_card-cashpulse.avif",imageAlt:"CashPulse cash flow forecast with 8-week prediction"},vero:{eyebrow:"AGENT \xB7 VERO",headline:"Vero is the AI agent <grad>that remembers everything.</grad>",claims:[{icon:"brain",text:"MemoryMesh \u2014 institutional memory across cases"},{icon:"sparkles",text:"Persistent agent that learns your ops over time"},{icon:"layers",text:"Works across cash app, collections, deductions, forecasting"},{icon:"check-circle",text:"Deploys inside your cloud \u2014 data never leaves"}],ctaText:"Meet Vero",href:"/solutions/vero-agent",imageUrl:"https://cdn.prod.website-files.com/684931abb239b84984296d93/69dfa29f0775fd254511fbd0_fbbe4ebdc1f2e22cbb6b2a6f0819e1cd_card-vero.avif",imageAlt:"Vero AI agent dashboard"},"dso-calc":{eyebrow:"FREE TOOL \xB7 DSO CALCULATOR",eyebrowColor:"#ff5043",headline:"See exactly how much cash <grad>your DSO is trapping.</grad>",claims:[{icon:"calculator",text:"Plug in revenue + AR, get your DSO in seconds"},{icon:"chart-up",text:"Compares against industry benchmark live"},{icon:"dollar",text:"Shows trapped cash in dollars, not just days"},{icon:"check-circle",text:"No signup required to calculate"}],ctaText:"Open the calculator",href:"/tools/dso-calculator",imageUrl:"https://cdn.prod.website-files.com/684931abb239b84984296d93/69eb8c31e3a4b8fa9eba9a77_dso-excel-preview-1.png",imageAlt:"DSO calculator preview"},"cf-template":{eyebrow:"FREE TOOL \xB7 13-WEEK CALCULATOR",eyebrowColor:"#ff5043",headline:"See your runway and tightest week, <grad>in seconds.</grad>",claims:[{icon:"clock",text:"Live runway and tightest week"},{icon:"chart-up",text:"Seven inputs, top-10 currencies"},{icon:"dollar",text:'"Pay 5 days faster" what-if slider'},{icon:"check-circle",text:"No signup to calculate"}],ctaText:"Open the calculator",href:"/tools/cash-flow-forecasting-tool",imageUrl:"https://cdn.prod.website-files.com/684931abb239b84984296d93/69ed0d37c19ca8bf38192eee_cash-forecast-excel-preview-1.png",imageAlt:"13-week cash flow forecast template preview"},demo:{eyebrow:"BOOK A CALL",headline:"See Transformance on <grad>your O2C data,</grad> in 30 minutes.",claims:[{icon:"sparkles",text:"Live demo against your remittance + invoice samples"},{icon:"check-circle",text:"No deck, no fluff \u2014 just the product on real data"},{icon:"clock",text:"30 minutes including Q&A"}],ctaText:"Book a Call",href:"/meeting",imageUrl:"https://cdn.prod.website-files.com/684931abb239b84984296d93/69d3b0701083034263a57a7e_685bc2f14cf8c5d87159c2fd_685bc2112a4dda2ba7ffa234_profile%20image%20paul%20hanke.avif",imageAlt:"Paul Hanke, Co-Founder of Transformance"}};function ne(s){return ie[s]||null}function oe(){return Object.keys(ie)}var re={clock:'<circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/>',"chart-up":'<path d="M3 3v18h18"/><path d="M7 14l4-4 4 4 5-5"/>',dollar:'<line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>',"check-circle":'<circle cx="12" cy="12" r="10"/><path d="M9 12l2 2 4-4"/>',shield:'<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>',zap:'<polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>',globe:'<circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>',layers:'<polygon points="12 2 2 7 12 12 22 7 12 2"/><polyline points="2 17 12 22 22 17"/><polyline points="2 12 12 17 22 12"/>',sparkles:'<path d="M12 3l1.5 4.5L18 9l-4.5 1.5L12 15l-1.5-4.5L6 9l4.5-1.5L12 3z"/><path d="M19 13l.75 2.25L22 16l-2.25.75L19 19l-.75-2.25L16 16l2.25-.75L19 13z"/>',calculator:'<rect x="4" y="2" width="16" height="20" rx="2"/><line x1="8" y1="6" x2="16" y2="6"/><line x1="8" y1="10" x2="8" y2="10"/><line x1="12" y1="10" x2="12" y2="10"/><line x1="16" y1="10" x2="16" y2="10"/><line x1="8" y1="14" x2="8" y2="14"/><line x1="12" y1="14" x2="12" y2="14"/><line x1="16" y1="14" x2="16" y2="14"/><line x1="8" y1="18" x2="8" y2="18"/><line x1="12" y1="18" x2="12" y2="18"/><line x1="16" y1="18" x2="16" y2="18"/>',phone:'<path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/>',receipt:'<path d="M16 2H8a2 2 0 0 0-2 2v18l3-2 3 2 3-2 3 2V4a2 2 0 0 0-2-2z"/><line x1="9" y1="7" x2="15" y2="7"/><line x1="9" y1="11" x2="15" y2="11"/>',wallet:'<path d="M19 7V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-2"/><path d="M21 12V7H6a2 2 0 0 1 0-4h13"/>',brain:'<path d="M12 2a4 4 0 0 0-4 4v1a4 4 0 0 0 0 8v3a4 4 0 1 0 8 0v-3a4 4 0 0 0 0-8V6a4 4 0 0 0-4-4z"/>'};function ce(s){return`<svg class="tfb__icon" viewBox="0 0 24 24">${re[s]||re["check-circle"]}</svg>`}function Me(s){return String(s).replace(/<grad>([\s\S]*?)<\/grad>/gi,'<span class="tfb__grad">$1</span>')}function V(s){return String(s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}var W=class extends HTMLElement{connectedCallback(){this._mounted||(this._mounted=!0,this._render())}static get observedAttributes(){return["data-topic","data-headline","data-cta-text","data-href","data-image-url","data-image-alt"]}attributeChangedCallback(){this._mounted&&this._render()}_render(){let e=this.dataset.topic,a=ne(e);if(!a){console.warn(`[transformance-banner] Unknown topic "${e}". Available: ${oe().join(", ")}`),this.style.display="none";return}let t=this.dataset.headline||a.headline,r=this.dataset.ctaText||a.ctaText,n=this.dataset.href||a.href,c=this.dataset.imageUrl||a.imageUrl,d=this.dataset.imageAlt||a.imageAlt||`${a.eyebrow} preview`,p=a.eyebrowColor?` style="color:${V(a.eyebrowColor)}"`:"",v=(a.claims||[]).map(b=>`<li>${ce(b.icon)}<span>${b.text}</span></li>`).join(""),x=Me(t),h=c?`
    ...
    `}};customElements.get("transformance-banner")||customElements.define("transformance-banner",W);var Te=/^\s*\[tf-banner:\s*([a-z0-9-]+)\s*\]\s*$/i;function le(s){if(!s)return;s.querySelectorAll("p").forEach(a=>{let t=(a.textContent||"").trim(),r=Te.exec(t);if(!r)return;let n=r[1].toLowerCase(),c=document.createElement("transformance-banner");c.setAttribute("data-topic",n),a.replaceWith(c)})}function de(){let s=document.querySelectorAll(".w-richtext");if(!s.length){le(document.body);return}s.forEach(le)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",de):de();})();
