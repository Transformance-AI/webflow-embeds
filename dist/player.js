...
`,ve={id:"cash-app",tag:"CASH APPLICATION \xB7 INTERACTIVE DEMO",coverLabel:"See cash application in 30 seconds",closing:{headline:`That's 90% of cash application, <span class="grad">automated</span>.`,sub:"Want to see it on your own data?"},scenes:[{id:"01-inbox",title:"Step 1 \xB7 Inbox",body:"Three remittances arrived overnight. Vero already started on the first.",tooltipSide:"right",spotlight:"#row-hero",advanceOn:{click:"#row-hero"},html:`${S}
      ...
      `}]};$(ve);function ge(s){return s>=36?"red":s>=16?"amber":"green"}var E=[{co:i.energy,balance:"\u20AC179K",days:45,last:"Last Fri"},{co:i.logistics,balance:"\u20AC92K",days:18,last:"Never"},{co:i.pharma,balance:"\u20AC47K",days:22,last:"Email sent"},{co:i.retail,balance:"\u20AC18K",days:60,last:"3d ago"},{co:i.tech,balance:"\u20AC22K",days:35,last:"Call Thu"},{co:i.food,balance:"\u20AC14K",days:28,last:"1w ago"},{co:i.motors,balance:"\u20AC63K",days:12,last:"Never"},{co:i.build,balance:"\u20AC38K",days:40,last:"Email sent"}],he="\u20AC475K",C=E.length,M=`
  <style>
    /* Base scene frame */
  ...
  `}var be={id:"collections",tag:"COLLECTIONS \xB7 INTERACTIVE DEMO",coverLabel:"See collections in 30 seconds",closing:{headline:'8 PTPs captured. 3 escalations routed. <span class="grad">Hours back.</span>',sub:"Want Vero to run your morning queue, every morning?"},scenes:[{id:"c-01-worklist",title:"Step 1 \xB7 Worklist",body:`${C} accounts overdue today. Vero works them all at once: calls in <span class="grad">their language</span>, emails in your tone.`,tooltipSide:"left",spotlight:"#run-batch",advanceOn:{click:"#run-batch"},html:`${M}
        <div class="head">
          <div>
            ...
            <div class="head-sub">${C} accounts \xB7 ${he} past due \xB7 updated 2 min ago</div>
          </div>
...
`;function J({weeks:s,highlight:e=-1,resolved:a=!1}){let h=m=>36+m/(s.length-1)*668,y=Math.max(...s)*1.1,b=Math.min(...s)*.85,f=m=>18+(1-(m-b)/(y-b))*174,l=3,g=s.slice(0,l).map((m,w)=>`${w===0?"M":"L"}${h(w).toFixed(1)},${f(m).toFixed(1)}`).join(" "),j=s.slice(l-1).map((m,w)=>`${w===0?"M":"L"}${h(w+l-1).toFixed(1)},${f(m).toFixed(1)}`).join(" "),L=s.slice(l-1).map((m,w)=>`${h(w+l-1).toFixed(1)},${f(m*1.1).toFixed(1)}`),D=s.slice(l-1).map((m,w)=>`${h(w+l-1).toFixed(1)},${f(m*.92).toFixed(1)}`).reverse(),B=`M${L.join(" L")} L${D.join(" L")} Z`,q=[0,3,6,9,12].map(m=>`<text x="${h(m)}" y="212" font-size="9" fill="rgba(10,10,10,0.45)" text-anchor="middle">W${m+1}</text>`).join(""),k=[0,.33,.66,1].map(m=>{let w=18+m*174;return`<line x1="36" y1="${w}" x2="704" y2="${w}" stroke="rgba(10,10,10,0.05)" stroke-width="1"/>`}).join(""),_=[y,y*.66+b*.34,y*.33+b*.67,b].map((m,w)=>`<text x="30" y="${18+w/3*174+3}" font-size="9" fill="rgba(10,10,10,0.4)" text-anchor="end">\u20AC${(m/1e3).toFixed(0)}K</text>`).join(""),I=e>=0?`
...
`;var X=[{main:"Your AR, on autopilot.",sub:"From chaos to clarity in seconds."},{main:"One agent. Every workflow.",sub:"Collections, payments, deductions, forecasts."},{main:"You ask. Vero delivers.",sub:"The AI finance agent that gets things done."}],_e=["\u20AC890K cleared, 3 JEs posted","PTP \u20AC179K confirmed for 25 Apr","\u20AC3,780 valid trade promo, \u20AC450 to review","\u20AC14.2M projected, +6% from new PTP"],Se=["Ask","Ingest","Analyze","Execute","Complete","Review","Report"],Ce=[3,7,5,9,4,8,6,10,3,7,5,8,4,6,9,5,7,3],ee=[{x1:195,y1:55,x2:370,y2:215},{x1:185,y1:170,x2:370,y2:235},{x1:190,y1:305,x2:370,y2:260},{x1:185,y1:420,x2:370,y2:280}],se=[{x1:530,y1:215,x2:660,y2:50},{x1:530,y1:235,x2:660,y2:165},{x1:530,y1:260,x2:660,y2:300},{x1:530,y1:280,x2:660,y2:405}],ae={x1:780,y1:225,x2:730,y2:400},je=[{p:0,d:0},{p:1,d:2200},{p:2,d:4e3},{p:3,d:5800},{p:4,d:7800},{p:5,d:9800},{p:6,d:13e3},{p:7,d:16500},{p:-1,d:2e4}],Ae=`
