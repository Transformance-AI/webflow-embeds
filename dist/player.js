...
`,ve={id:"cash-app",tag:"CASH APPLICATION \xB7 INTERACTIVE DEMO",coverLabel:"See cash application in 30 seconds",closing:{headline:`That's 90% of cash application, <span class="grad">automated</span>.`,sub:"Want to see it on your own data?"},scenes:[{id:"01-inbox",title:"Step 1 \xB7 Inbox",body:"Three remittances arrived overnight. Vero already started on the first.",tooltipSide:"right",spotlight:"#row-hero",advanceOn:{click:"#row-hero"},html:`${j}
      ...
      `}]};$(ve);function ge(s){return s>=36?"red":s>=16?"amber":"green"}var E=[{co:i.energy,balance:"\u20AC179K",days:45,last:"Last Fri"},{co:i.logistics,balance:"\u20AC92K",days:18,last:"Never"},{co:i.pharma,balance:"\u20AC47K",days:22,last:"Email sent"},{co:i.retail,balance:"\u20AC18K",days:60,last:"3d ago"},{co:i.tech,balance:"\u20AC22K",days:35,last:"Call Thu"},{co:i.food,balance:"\u20AC14K",days:28,last:"1w ago"},{co:i.motors,balance:"\u20AC63K",days:12,last:"Never"},{co:i.build,balance:"\u20AC38K",days:40,last:"Email sent"}],he="\u20AC475K",S=E.length,M=`
  <style>
    /* Base scene frame */
  ...
  `}var be={id:"collections",tag:"COLLECTIONS \xB7 INTERACTIVE DEMO",coverLabel:"See collections in 30 seconds",closing:{headline:'8 PTPs captured. 3 escalations routed. <span class="grad">Hours back.</span>',sub:"Want Vero to run your morning queue, every morning?"},scenes:[{id:"c-01-worklist",title:"Step 1 \xB7 Worklist",body:`${S} accounts overdue today. Vero works them all at once: calls in <span class="grad">their language</span>, emails in your tone.`,tooltipSide:"left",spotlight:"#run-batch",advanceOn:{click:"#run-batch"},html:`${M}
        <div class="head">
          <div>
            ...
            <div class="head-sub">${S} accounts \xB7 ${he} past due \xB7 updated 2 min ago</div>
          </div>
...
`;function J({weeks:s,highlight:e=-1,resolved:a=!1}){let h=m=>36+m/(s.length-1)*668,y=Math.max(...s)*1.1,b=Math.min(...s)*.85,u=m=>18+(1-(m-b)/(y-b))*174,l=3,g=s.slice(0,l).map((m,w)=>`${w===0?"M":"L"}${h(w).toFixed(1)},${u(m).toFixed(1)}`).join(" "),C=s.slice(l-1).map((m,w)=>`${w===0?"M":"L"}${h(w+l-1).toFixed(1)},${u(m).toFixed(1)}`).join(" "),L=s.slice(l-1).map((m,w)=>`${h(w+l-1).toFixed(1)},${u(m*1.1).toFixed(1)}`),D=s.slice(l-1).map((m,w)=>`${h(w+l-1).toFixed(1)},${u(m*.92).toFixed(1)}`).reverse(),B=`M${L.join(" L")} L${D.join(" L")} Z`,q=[0,3,6,9,12].map(m=>`<text x="${h(m)}" y="212" font-size="9" fill="rgba(10,10,10,0.45)" text-anchor="middle">W${m+1}</text>`).join(""),k=[0,.33,.66,1].map(m=>{let w=18+m*174;return`<line x1="36" y1="${w}" x2="704" y2="${w}" stroke="rgba(10,10,10,0.05)" stroke-width="1"/>`}).join(""),_=[y,y*.66+b*.34,y*.33+b*.67,b].map((m,w)=>`<text x="30" y="${18+w/3*174+3}" font-size="9" fill="rgba(10,10,10,0.4)" text-anchor="end">\u20AC${(m/1e3).toFixed(0)}K</text>`).join(""),I=e>=0?`
  ...
  `}var ke={id:"predictions",tag:"CASH FLOW FORECASTING \xB7 INTERACTIVE DEMO",coverLabel:"See cash predictions in 30 seconds",closing:{headline:'A \u20AC170K dip avoided. Forecast back on track. <span class="grad">Hours back.</span>',sub:"Want Vero to monitor and act on signals every morning?"},scenes:[{id:"p-01-forecast",title:"Step 1 \xB7 13-week forecast",body:'Cash holding strong, except a <span class="grad">\u20AC170K dip</span> in week 6. Vero spotted the risk before it lands.',tooltipSide:"left",spotlight:"#dip-investigate",advanceOn:{click:"#dip-investigate"},html:`${P}
      ...
      `},{id:"p-03-atrisk",title:"Step 3 \xB7 At-risk invoices",body:`${i.energy.name}'s \u20AC179K is the biggest exposure. <span class="grad">88% likely to slip</span> based on past behaviour plus signals.`,tooltipSide:"bottom",spotlight:"#energy-row",advanceOn:{click:"#energy-row"},html:`${P}
        ...
        <div class="card">
          ...
          ${ue.map((s,e)=>{let a=e===0,t=s.risk>=80?"red":s.risk>=65?"amber":"gray",r=s.risk>=80?"#ef4444":s.risk>=65?"#f59e0b":"#94a3b8";return`
      ...
      `},{id:"p-05-resolved",title:"Step 5 \xB7 Forecast restored",body:'Dip mitigated. Forecast back on trend. Want Vero to <span class="grad">monitor and act</span> on these signals every morning?',tooltipSide:"top",spotlight:"#pred-yes",advanceOn:{click:"#pred-yes"},html:`${P}
        ...
        <div class="chart-card">
          ...
          ${J({weeks:fe,highlight:6,resolved:!0})}
        </div>
