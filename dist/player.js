...
`,Oe={id:"cash-app",tag:"CASH APPLICATION \xB7 INTERACTIVE DEMO",coverLabel:"See cash application in 30 seconds",closing:{headline:`That's 90% of cash application, <span class="grad">automated</span>.`,sub:"Want to see it on your own data?"},scenes:[{id:"01-inbox",title:"Step 1 \xB7 Inbox",body:"Three remittances arrived overnight. Vero already started on the first.",tooltipSide:"right",spotlight:"#row-hero",advanceOn:{click:"#row-hero"},html:`${E}
      ...
      `}]};A(Oe);function Ke(s){return s>=36?"red":s>=16?"amber":"green"}var D=[{co:n.energy,balance:"\u20AC179K",days:45,last:"Last Fri"},{co:n.logistics,balance:"\u20AC92K",days:18,last:"Never"},{co:n.pharma,balance:"\u20AC47K",days:22,last:"Email sent"},{co:n.retail,balance:"\u20AC18K",days:60,last:"3d ago"},{co:n.tech,balance:"\u20AC22K",days:35,last:"Call Thu"},{co:n.food,balance:"\u20AC14K",days:28,last:"1w ago"},{co:n.motors,balance:"\u20AC63K",days:12,last:"Never"},{co:n.build,balance:"\u20AC38K",days:40,last:"Email sent"}],Ne="\u20AC475K",_=D.length,R=`
  <style>
    /* Base scene frame */
...
`;function V(s,e={}){let{status:a="",dim:t=!1,highlight:o=!1,id:i=""}=e,c=["wl-row"];t&&c.push("dim"),o&&c.push("highlight");let p=a||`<span style="font-size: 11px; color: rgba(10,10,10,0.5);">${s.last}</span>`,g=Ke(s.days);return`
...
`,Fe={id:"deductions",tag:"DEDUCTIONS \xB7 INTERACTIVE DEMO",coverLabel:"See deductions in 30 seconds",closing:{headline:'Short-pay investigated, settlement posted. <span class="grad">Hours saved.</span>',sub:"Want Vero to auto-resolve patterns it's already seen?"},scenes:[{id:"d-01-queue",title:"Step 1 \xB7 Deduction queue",body:`${n.retail.name} paid short by <span class="grad">\u20AC4,230</span> on a \u20AC92K invoice. Vero will pull the evidence.`,tooltipSide:"left",spotlight:"#ded-investigate",advanceOn:{click:"#ded-investigate"},html:`${Z}
      ...
      `}]};A(Fe);var qe=[{co:n.energy,inv:"INV-0019",amount:179e3,risk:88,reason:"2 broken PTPs \xB7 sentiment declining"},{co:n.motors,inv:"INV-1183",amount:63e3,risk:71,reason:"DSO trend +9 days \xB7 last call 12d ago"},{co:n.fashion,inv:"INV-0876",amount:42e3,risk:64,reason:"New CFO \xB7 payment behavior unclear"},{co:n.tech,inv:"INV-2204",amount:22e3,risk:58,reason:"Subscription dispute open"},{co:n.build,inv:"INV-1144",amount:38e3,risk:52,reason:"POD discrepancy under review"}],q=`
  <style>
...
`;function ge({weeks:s,highlight:e=-1,resolved:a=!1}){let v=k=>36+k/(s.length-1)*668,h=Math.max(...s)*1.1,u=Math.min(...s)*.85,b=k=>18+(1-(k-u)/(h-u))*174,l=3,m=s.slice(0,l).map((k,S)=>`${S===0?"M":"L"}${v(S).toFixed(1)},${b(k).toFixed(1)}`).join(" "),j=s.slice(l-1).map((k,S)=>`${S===0?"M":"L"}${v(S+l-1).toFixed(1)},${b(k).toFixed(1)}`).join(" "),N=s.slice(l-1).map((k,S)=>`${v(S+l-1).toFixed(1)},${b(k*1.1).toFixed(1)}`),Q=s.slice(l-1).map((k,S)=>`${v(S+l-1).toFixed(1)},${b(k*.92).toFixed(1)}`).reverse(),U=`M${N.join(" L")} L${Q.join(" L")} Z`,G=[0,3,6,9,12].map(k=>`<text x="${v(k)}" y="212" font-size="9" fill="rgba(10,10,10,0.45)" text-anchor="middle">W${k+1}</text>`).join(""),w=[0,.33,.66,1].map(k=>{let S=18+k*174;return`<line x1="36" y1="${S}" x2="704" y2="${S}" stroke="rgba(10,10,10,0.05)" stroke-width="1"/>`}).join(""),C=[h,h*.66+u*.34,h*.33+u*.67,u].map((k,S)=>`<text x="30" y="${18+S/3*174+3}" font-size="9" fill="rgba(10,10,10,0.4)" text-anchor="end">\u20AC${(k/1e3).toFixed(0)}K</text>`).join(""),B=e>=0?`
  ...
  `}function ae(s,e){let a=He[s],t=e||a.defaultIntensity,o=a.weeks(t),i=a.risk(t);return`
    <div class="sc-head">
      <div>
  ...
  `}var Ge={id:"predictions",tag:"CASH FLOW FORECASTING \xB7 INTERACTIVE DEMO",coverLabel:"See cash predictions in 30 seconds",closing:{headline:'A \u20AC170K dip avoided. Forecast back on track. <span class="grad">Hours back.</span>',sub:"Want Vero to monitor and act on signals every morning?"},scenes:[{id:"p-01-forecast",title:"Step 1 \xB7 13-week forecast",body:'Cash holding strong, except a <span class="grad">\u20AC170K dip</span> in week 6. Vero spotted the risk before it lands.',tooltipSide:"left",spotlight:"#dip-investigate",advanceOn:{click:"#dip-investigate"},html:`${q}
      ...
      `},{id:"p-03-atrisk",title:"Step 3 \xB7 At-risk invoices",body:`${n.energy.name}'s \u20AC179K is the biggest exposure. <span class="grad">88% likely to slip</span> based on past behaviour plus signals.`,tooltipSide:"bottom",spotlight:"#energy-row",advanceOn:{click:"#energy-row"},html:`${q}
        <style>
        ...
        <div class="card">
          ...
          ${qe.map((s,e)=>{let a=e===0,t=s.risk>=80?"red":s.risk>=65?"amber":"gray",o=s.risk>=80?"#ef4444":s.risk>=65?"#f59e0b":"#94a3b8";return`
...
`,te=class extends HTMLElement{constructor(){super(),this._phase=-1,this._cycle=0,this._timers=[],this._avatarTimers=[],this._started=!1,this._paused=!1,this._observer=null}connectedCallback(){if(this._shadow)return;this._shadow=this.attachShadow({mode:"open"}),this._shadow.innerHTML=`<style>${he}</style>${ts}`,this._q=t=>this._shadow.querySelector(t),this.style.transform="none",this.style.margin="0",this._buildStaticBits(),this._fitEls={fit:this._q(".canvas-fit"),wrap:this._q(".canvas-wrap")},this._fit(),"ResizeObserver"in window?(this._ro=new ResizeObserver(()=>this._fit()),this._ro.observe(this.parentElement||this)):(this._onResize=()=>this._fit(),window.addEventListener("resize",this._onResize));let e=()=>{this._started||(this._started=!0,this._runCycle())},a=()=>{"requestIdleCallback"in window?window.requestIdleCallback(e,{timeout:500}):setTimeout(e,200)};"IntersectionObserver"in window?(this._observer=new IntersectionObserver(t=>{t[0].isIntersecting?(this._paused=!1,this._started||a()):this._paused=!0},{threshold:.01}),this._observer.observe(this)):a()}disconnectedCallback(){this._timers.forEach(clearTimeout),this._timers=[],this._clearAvatarTimers(),this._observer?.disconnect(),this._ro?.disconnect(),this._onResize&&window.removeEventListener("resize",this._onResize)}_fit(){let e=this._fitEls;if(!e||!e.fit||!e.wrap)return;let a=this.parentElement?this.parentElement.clientWidth:0,t=Math.min(920,a||e.fit.clientWidth||560);e.fit.style.width=t+"px";let o=Math.min(1,t/920);e.fit.style.height=Math.round(500*o)+"px",e.wrap.style.transform=`scale(${o})`}_buildStaticBits(){let e=this._q("#waveform");ss.forEach((i,c)=>{let p=document.createElement("div");p.className="waveform-bar",p.setAttribute("data-h",i),p.style.transitionDelay=c*30+"ms",e.appendChild(p)});let a=this._q("#progress-steps");es.forEach((i,c)=>{let p=document.createElement("div");p.className="progress-step",p.innerHTML=`<div class="progress-dot" data-step="${c}"></div><span class="progress-label" data-step="${c}">${i}</span>`,a.appendChild(p)});let t=i=>{let c=(i.x1+i.x2)/2,p=(i.x1+i.x2)/2;return`M${i.x1},${i.y1} C${c},${i.y1} ${p},${i.y2} ${i.x2},${i.y2}`},o=i=>Math.sqrt(Math.pow(i.x2-i.x1,2)+Math.pow(i.y2-i.y1,2))*1.5;this._allConns=[];for(let i=0;i<4;i++){let c=this._q(`#conn-in-${i}`),p=t(be[i]),g=o(be[i]);c.setAttribute("d",p),c.style.strokeDasharray=g,c.style.strokeDashoffset=g,c.style.transition=`stroke-dashoffset 0.8s cubic-bezier(0.22,1,0.36,1) ${i*.12}s`,this._allConns.push({el:c,len:g,phase:2})}for(let i=0;i<4;i++){let c=this._q(`#conn-out-${i}`),p=t(fe[i]),g=o(fe[i]);c.setAttribute("d",p),c.style.strokeDasharray=g,c.style.strokeDashoffset=g,c.style.transition=`stroke-dashoffset 0.8s cubic-bezier(0.22,1,0.36,1) ${i*.12}s`,this._allConns.push({el:c,len:g,phase:3})}{let i=this._q("#conn-cross"),c=t(me),p=o(me);i.setAttribute("d",c),i.style.strokeDasharray=p,i.style.strokeDashoffset=p,i.style.transition="stroke-dashoffset 0.8s cubic-bezier(0.22,1,0.36,1) 0.3s",this._allConns.push({el:i,len:p,phase:4})}this._drawMiniChart(!1)}_drawMiniChart(e){let a=[6,5.5,6.2,5.8,6.5,7,e?7.8:7.2],t=130,o=40,i=4,c=8.5,p=4.5,g=u=>i+u/(a.length-1)*(t-2*i),f=u=>i+(c-u)/(c-p)*(o-2*i),x=a.map((u,b)=>(b===0?"M":"L")+g(b)+","+f(u)).join(" "),v=this._q("#mini-chart"),h=`<path d="${x}" fill="none" stroke="${e?"#6366f1":"#d1d5db"}" stroke-width="2" stroke-linecap="round" style="transition:all 0.8s ease"/>`;e&&(h+=`<path d="M${g(5)},${f(7)} L${g(6)},${f(7.8)} L${g(6)},${f(7.2)} L${g(5)},${f(7)} Z" fill="#818cf8" opacity="0.2"/>`,h+=`<circle cx="${g(6)}" cy="${f(7.8)}" r="3" fill="#6366f1"/>`,h+=`<line x1="${g(5)}" y1="${f(7.2)}" x2="${g(6)}" y2="${f(7.2)}" stroke="#d1d5db" stroke-width="1" stroke-dasharray="3 2"/>`),v.innerHTML=h}_clearAvatarTimers(){this._avatarTimers.forEach(e=>{clearTimeout(e),clearInterval(e)}),this._avatarTimers=[]}_animateVeroAvatar(e,a){if(!e)return;let t=e.querySelector(".vero-avatar-text");if(!t)return;let o=t.querySelectorAll("span");if(a==="idle"){t.style.color="#10B981",o[0].textContent="0",o[1].textContent="0";let i=()=>{o[0].textContent="-",this._avatarTimers.push(setTimeout(()=>{o[0].textContent="0"},150)),this._avatarTimers.push(setTimeout(i,700+Math.random()*2e3))};this._avatarTimers.push(setTimeout(i,700+Math.random()*1e3));let c=()=>{o[1].textContent="-",this._avatarTimers.push(setTimeout(()=>{o[1].textContent="0"},150)),this._avatarTimers.push(setTimeout(c,1e3+Math.random()*2500))};this._avatarTimers.push(setTimeout(c,1e3+Math.random()*1500))}else if(a==="thinking"){t.style.color="#6EE7B7";let i=setInterval(()=>{o[0].textContent=Math.random()>.5?"1":"0",o[1].textContent=Math.random()>.5?"1":"0"},80);this._avatarTimers.push(i)}else a==="done"&&(t.style.color="#10B981",o[0].textContent="^",o[1].textContent="^")}_setPhase(e){this._phase=e,this._clearAvatarTimers();let a=e===0||e===6,t=e>=1&&e<=4,o=e===5,i=e===7;this._q("#chat-overlay").classList.toggle("visible",a);let p=this._q("#chat-human");p.className="chat-human"+(e===0?" visible":e===6?" faded":""),p.style.transitionDelay=e===0?"0.3s":"0s";let g=this._q("#chat-thinking");g.className="chat-vero-thinking"+(e===0?" visible":""),g.style.transitionDelay=e===0?"1s":"0s";let f=this._q("#chat-summary");f.className="chat-vero-summary"+(e===6?" visible":""),f.style.transitionDelay=e===6?"0.5s":"0s",e===0&&this._animateVeroAvatar(this._q("#vero-avatar-chat-thinking"),"thinking"),e===6&&this._animateVeroAvatar(this._q("#vero-avatar-chat-summary"),"done"),this._q("#canvas-layer").classList.toggle("visible",t);for(let l=0;l<4;l++)this._q("#input-"+l).classList.toggle("visible",e>=1);this._allConns.forEach(l=>{l.el.style.strokeDashoffset=e>=l.phase?"0":l.len});let x=this._q("#vero-hub");x.classList.toggle("visible",e>=2),x.classList.toggle("pulsing",e>=2&&e<=4),this._q("#vero-hub-card").classList.toggle("active",e>=3);let v=this._q("#vero-hub-status");v.textContent=e<3?d.statusAnalyzing:d.statusRunning,v.style.color=e>=3?"var(--violet-600)":"var(--gray-400)",e>=2&&this._animateVeroAvatar(this._q("#vero-avatar-hub"),e>=3?"thinking":"idle");for(let l=0;l<4;l++){let m=this._q("#output-"+l);m.classList.toggle("visible",e>=3),m.classList.toggle("done",e>=4),this._q("#check-"+l).classList.toggle("visible",e>=4)}for(let l=0;l<3;l++)this._q("#inv-"+l).classList.toggle("done",e>=4),this._q("#badge-"+l).classList.toggle("done",e>=4);this._q("#cleared-text").classList.toggle("visible",e>=4),this._q("#waveform").querySelectorAll(".waveform-bar").forEach(l=>{let m=e>=3;l.classList.toggle("active",m);let j=parseFloat(l.getAttribute("data-h")),N=m?Math.min(1,j*2.2/22):.14;l.style.transform=`scaleY(${N})`}),this._q("#call-detail").classList.toggle("visible",e>=4);for(let l=0;l<3;l++){let m=this._q("#ev-check-"+l);m.classList.toggle("done",e>=4),m.classList.toggle("pending",e<4),m.style.transitionDelay=l*150+400+"ms"}if(this._q("#verdict-fill").classList.toggle("done",e>=4),this._drawMiniChart(e>=4),this._q("#forecast-uplift").classList.toggle("visible",e>=4),this._q("#checklist-overlay").classList.toggle("visible",o),o){this._animateVeroAvatar(this._q("#vero-avatar-checklist"),"thinking"),this._resetTodos();for(let l=0;l<4;l++)(m=>{this._timers.push(setTimeout(()=>this._checkTodo(m),600+m*500))})(l)}if(this._q("#tagline-overlay").classList.toggle("visible",i),i){let l=ve[this._cycle%ve.length];this._q("#tagline-content").innerHTML=`<div class="tagline-main">${l.main}</div><div class="tagline-sub">${l.sub}</div>`}let b=Math.min(100,e/7*100);this._q("#progress-fill").style.width=b+"%",this._q("#progress-steps").querySelectorAll(".progress-dot").forEach(l=>{let m=parseInt(l.getAttribute("data-step"));l.className="progress-dot"+(e>=m?m<=1?" active-v":m<=4?" active-e":" active-b":"")}),this._q("#progress-steps").querySelectorAll(".progress-label").forEach(l=>{let m=parseInt(l.getAttribute("data-step"));l.classList.toggle("active",e>=m)})}_resetTodos(){for(let e=0;e<4;e++)this._q("#todo-box-"+e).className="check-box",this._q("#todo-box-"+e).innerHTML="",this._q("#todo-label-"+e).className="check-label",this._q("#todo-sub-"+e).className="check-sub",this._q("#todo-sub-"+e).textContent=d.todoProcessing;this._q("#checklist-footer").classList.remove("visible")}_checkTodo(e){let a=this._q("#todo-box-"+e);a.className="check-box checked",a.innerHTML='<svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M2.5 6L5 8.5L9.5 3.5" stroke="white" stroke-width="1.5" stroke-linecap="round"/></svg>',this._q("#todo-label-"+e).classList.add("checked");let t=this._q("#todo-sub-"+e);t.classList.add("checked"),t.textContent=Xe[e],e===3&&this._q("#checklist-footer").classList.add("visible")}_runCycle(){this._timers.forEach(clearTimeout),this._timers=[],as.forEach(e=>{this._timers.push(setTimeout(()=>{this._paused||(e.p===-1?(this._cycle++,this._runCycle()):this._setPhase(e.p))},e.d))})}};customElements.get("transformance-hero")||customElements.define("transformance-hero",te);var xe=`
:host{
  --tf-canvas:#f6f1ea;
  --tf-canvas-2:#efe6da;
  --tf-ink:#0a0a0a;
