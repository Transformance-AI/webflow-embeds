...
`;var ne={};function $(s){ne[s.id]=s,document.querySelectorAll(`transformance-tour[data-tour="${s.id}"]`).forEach(e=>{e._mounted||e._tryMount()})}var Y=class extends HTMLElement{constructor(){super(),this._mounted=!1,this._stepIndex=0,this._tour=null}connectedCallback(){this._mounted||this._tryMount()}_tryMount(){let e=this.dataset.tour,a=e&&ne[e];a&&(this._mounted=!0,this._tour=a,this._shadow=this.attachShadow({mode:"open"}),this._shadow.innerHTML=`
      <style>${ie}</style>
...
`,Ae={id:"cash-app",tag:"CASH APPLICATION \xB7 INTERACTIVE DEMO",coverLabel:"See cash application in 30 seconds",closing:{headline:`That's 90% of cash application, <span class="grad">automated</span>.`,sub:"Want to see it on your own data?"},scenes:[{id:"01-inbox",title:"Step 1 \xB7 Inbox",body:"Three remittances arrived overnight. Vero already started on the first.",tooltipSide:"right",spotlight:"#row-hero",advanceOn:{click:"#row-hero"},html:`${C}
      ...
      `},{id:"02-extract",title:"Step 2 \xB7 Extraction",body:'Every field extracted with <span class="grad">confidence</span>. No templates to maintain.',tooltipSide:"top",spotlight:"#extraction-table",html:`${C}
        <style>
      ...
      `},{id:"03-match",title:"Step 3 \xB7 Auto-match",body:'Three invoices, three matches. <span class="grad">\u20AC530K accounted for</span> in under 4 seconds.',tooltipSide:"right",spotlight:"#match-cards",html:`${C}
        <style>
      ...
      `},{id:"04-edge",title:"Step 4 \xB7 Edge case",body:`This one's borderline. Vero surfaces it, <span class="grad">you decide</span> in one click.`,tooltipSide:"left",spotlight:"#edge-card",advanceOn:{click:"#edge-confirm"},html:`${C}
        <style>
      ...
      `},{id:"05-je",title:"Step 5 \xB7 Journal entry",body:'Journal entry assembled, audit trail attached. One click to <span class="grad">post to ERP</span>.',tooltipSide:"top",spotlight:"#post-btn",advanceOn:{click:"#post-btn"},html:`${C}
        <style>
      ...
      `},{id:"06-posted",title:"Step 6 \xB7 Posted",body:"Vero noticed this is a daily pattern. Want it to handle every morning?",tooltipSide:"right",spotlight:"#vero-card",advanceOn:{click:"#vero-yes"},html:`${C}
        <style>
      ...
      `},{id:"07-schedule",title:"Step 7 \xB7 Schedule",body:'Vero proposes the rules. <span class="grad">You stay in control</span> of every threshold.',tooltipSide:"left",spotlight:"#approve-btn",advanceOn:{click:"#approve-btn"},html:`${C}
        <style>
      ...
      `},{id:"08-done",title:"Step 8 \xB7 Live",body:'Done. Vero handles it tomorrow at <span class="grad">8:00 AM</span>, and every morning after.',tooltipSide:"right",spotlight:"#tile",html:`${C}
        <style>
      ...
      `}]};$(Ae);function je(s){return s>=36?"red":s>=16?"amber":"green"}var L=[{co:n.energy,balance:"\u20AC179K",days:45,last:"Last Fri"},{co:n.logistics,balance:"\u20AC92K",days:18,last:"Never"},{co:n.pharma,balance:"\u20AC47K",days:22,last:"Email sent"},{co:n.retail,balance:"\u20AC18K",days:60,last:"3d ago"},{co:n.tech,balance:"\u20AC22K",days:35,last:"Call Thu"},{co:n.food,balance:"\u20AC14K",days:28,last:"1w ago"},{co:n.motors,balance:"\u20AC63K",days:12,last:"Never"},{co:n.build,balance:"\u20AC38K",days:40,last:"Email sent"}],Te="\u20AC475K",A=L.length,E=`
  <style>
    /* Base scene frame */
...
`;function M(s,e={}){let{status:a="",dim:t=!1,highlight:o=!1,id:i=""}=e,c=["wl-row"];t&&c.push("dim"),o&&c.push("highlight");let p=a||`<span style="font-size: 11px; color: rgba(10,10,10,0.5);">${s.last}</span>`,d=je(s.days);return`
    <div class="${c.join(" ")}"${i?` id="${i}"`:""}>
      ...
      <span class="wl-flag" title="${s.co.country}">${oe(s.co.country)}</span>
      <span class="wl-bal mono">${s.balance}</span>
      <span class="wl-days"><span class="pill ${d}">${s.days}d</span></span>
      <span class="wl-status">${p}</span>
    </div>
  `}var Ee={id:"collections",tag:"COLLECTIONS \xB7 INTERACTIVE DEMO",coverLabel:"See collections in 30 seconds",closing:{headline:'8 PTPs captured. 3 escalations routed. <span class="grad">Hours back.</span>',sub:"Want Vero to run your morning queue, every morning?"},scenes:[{id:"c-01-worklist",title:"Step 1 \xB7 Worklist",body:`${A} accounts overdue today. Vero works them all at once: calls in <span class="grad">their language</span>, emails in your tone.`,tooltipSide:"left",spotlight:"#run-batch",advanceOn:{click:"#run-batch"},html:`${E}
        <div class="head">
          <div>
            <h2>Overdue worklist</h2>
            <div class="head-sub">${A} accounts \xB7 ${Te} past due \xB7 updated 2 min ago</div>
          </div>
      ...
      `},{id:"c-02-batch",title:"Step 2 \xB7 Batch running",body:`One queue, ${A} parallel workstreams. Vero follows your <span class="grad">playbook</span> for each: channel, language, escalation rules.`,tooltipSide:"left",spotlight:"#wl-running",html:`${E}
        <style>
        ...
        <div class="head">
          <div>
            <h2>Batch running</h2>
            <div class="head-sub">${A} accounts in flight \xB7 calls in native language, emails localized</div>
          </div>
        ...
        <div class="card" id="wl-running">
          ${L.map((s,e)=>{let a=['<span class="pill green"><span>\u2713</span> PTP \u20AC179K</span>','<span class="pill blue"><span>\u2709</span> Email sent</span>','<span class="pill amber"><span>\u26A0</span> Escalated</span>','<span class="pill amber"><span>\u26A0</span> Escalated</span>','<span class="pill amber"><span>\u26A0</span> Escalated</span>','<span class="pill green"><span>\u2713</span> PTP \u20AC14K</span>','<span class="pill green"><span>\u2713</span> Paid today</span>','<span class="pill blue"><span>\u2709</span> Reminder sent</span>'][e],o=`
              <div class="p-stages">
            ...
            `;return M(s,{status:o}).replace('<div class="wl-row',`<div data-idx="${e}" class="wl-row`)}).join("")}
        </div>
      `},{id:"c-03-ptp",title:"Step 3 \xB7 Promise to pay",body:`Maria at ${n.energy.name} committed <span class="grad">\u20AC179K</span> by Friday. Logged, with the full transcript attached.`,tooltipSide:"left",spotlight:"#ptp-card",html:`${E}
        <style>
          ...
          <div class="card">
            <div class="card-head">
              <div>
                ...
                <div class="card-meta">${A} accounts \xB7 1 focused</div>
              </div>
            </div>
            ${L.map((s,e)=>M(s,{dim:e!==0,status:e===0?'<span class="pill green">PTP \u20AC179K</span>':""})).join("")}
          </div>
          <div id="ptp-card">
            <div class="call-head">
              ${x(n.energy)}
      ...
      `},{id:"c-04-fallback",title:"Step 4 \xB7 Fallback",body:'No answer? Vero switches to email, in their language, <span class="grad">per your rules</span>, no prompt required.',tooltipSide:"left",spotlight:"#fallback-card",html:`${E}
        <style>
        ...
        <div class="wrap">
          <div class="card">
            ...
            ${L.map((s,e)=>M(s,{dim:e!==1,status:e===1?'<span class="pill blue">Email sent</span>':""})).join("")}
          </div>
      ...
      `},{id:"c-05-escalation",title:"Step 5 \xB7 Escalation",body:`Three accounts Vero shouldn't auto-resolve. Each lands in <span class="grad">your queue</span> with the full evidence pack.`,tooltipSide:"left",spotlight:"#esc-review-btn",advanceOn:{click:"#esc-review-btn"},html:`${E}
        <style>
        ...
        <div class="head" style="margin-bottom: 10px;">
          <div>
            <h2>Escalations</h2>
            <div class="head-sub">${A} accounts processed \xB7 3 need your call</div>
          </div>
        ...
        <div class="card">
          ${L.map((s,e)=>{if(!(e>=2&&e<=4))return M(s,{dim:!0,status:e===0?'<span class="pill green">PTP \u20AC179K</span>':e===1?'<span class="pill blue">Email sent</span>':e===5?'<span class="pill green">PTP \u20AC14K</span>':e===6?'<span class="pill green">Paid today</span>':'<span class="pill blue">Reminder sent</span>'});let o=`
            ...
            `;return M(s,{status:o}).replace('class="wl-row"','class="wl-row esc"')}).join("")}
        </div>
      `},{id:"c-06-done",title:"Step 6 \xB7 Done",body:'Eight minutes of Vero. Hours saved, <span class="grad">every morning</span>, if you want.',tooltipSide:"right",spotlight:"#coll-vero-card",advanceOn:{click:"#coll-yes"},html:`${E}
        <style>
...
`,Me={id:"deductions",tag:"DEDUCTIONS \xB7 INTERACTIVE DEMO",coverLabel:"See deductions in 30 seconds",closing:{headline:'Short-pay investigated, settlement posted. <span class="grad">Hours saved.</span>',sub:"Want Vero to auto-resolve patterns it's already seen?"},scenes:[{id:"d-01-queue",title:"Step 1 \xB7 Deduction queue",body:`${n.retail.name} paid short by <span class="grad">\u20AC4,230</span> on a \u20AC92K invoice. Vero will pull the evidence.`,tooltipSide:"left",spotlight:"#ded-investigate",advanceOn:{click:"#ded-investigate"},html:`${H}
      ...
      `},{id:"d-02-evidence",title:"Step 2 \xB7 Evidence chain",body:'Vero pulls the invoice, purchase order, proof of delivery, contract, and any active <span class="grad">trade-promo agreement</span>, then links them automatically.',tooltipSide:"bottom",spotlight:"#evidence-chain",html:`${H}
        <style>
      ...
      `},{id:"d-03-recommend",title:"Step 3 \xB7 Vero's plan",body:'Evidence assembled, plan drafted. <span class="grad">One click</span> to apply.',tooltipSide:"left",spotlight:"#apply-btn",advanceOn:{click:"#apply-btn"},html:`${H}
        <style>
      ...
      `},{id:"d-04-done",title:"Step 4 \xB7 Posted",body:'Settled in under 5 minutes. Vero spotted <span class="grad">14 similar cases</span> this quarter. Want Vero to resolve those automatically?',tooltipSide:"right",spotlight:"#ded-vero-card",advanceOn:{click:"#ded-yes"},html:`${H}
        <style>
      ...
      `}]};$(Me);var Le=[{co:n.energy,inv:"INV-0019",amount:179e3,risk:88,reason:"2 broken PTPs \xB7 sentiment declining"},{co:n.motors,inv:"INV-1183",amount:63e3,risk:71,reason:"DSO trend +9 days \xB7 last call 12d ago"},{co:n.fashion,inv:"INV-0876",amount:42e3,risk:64,reason:"New CFO \xB7 payment behavior unclear"},{co:n.tech,inv:"INV-2204",amount:22e3,risk:58,reason:"Subscription dispute open"},{co:n.build,inv:"INV-1144",amount:38e3,risk:52,reason:"POD discrepancy under review"}],R=`
  <style>
...
`;function re({weeks:s,highlight:e=-1,resolved:a=!1}){let g=y=>36+y/(s.length-1)*668,b=Math.max(...s)*1.1,u=Math.min(...s)*.85,m=y=>18+(1-(y-u)/(b-u))*174,l=3,f=s.slice(0,l).map((y,w)=>`${w===0?"M":"L"}${g(w).toFixed(1)},${m(y).toFixed(1)}`).join(" "),j=s.slice(l-1).map((y,w)=>`${w===0?"M":"L"}${g(w+l-1).toFixed(1)},${m(y).toFixed(1)}`).join(" "),V=s.slice(l-1).map((y,w)=>`${g(w+l-1).toFixed(1)},${m(y*1.1).toFixed(1)}`),W=s.slice(l-1).map((y,w)=>`${g(w+l-1).toFixed(1)},${m(y*.92).toFixed(1)}`).reverse(),F=`M${V.join(" L")} L${W.join(" L")} Z`,K=[0,3,6,9,12].map(y=>`<text x="${g(y)}" y="212" font-size="9" fill="rgba(10,10,10,0.45)" text-anchor="middle">W${y+1}</text>`).join(""),k=[0,.33,.66,1].map(y=>{let w=18+y*174;return`<line x1="36" y1="${w}" x2="704" y2="${w}" stroke="rgba(10,10,10,0.05)" stroke-width="1"/>`}).join(""),_=[b,b*.66+u*.34,b*.33+u*.67,u].map((y,w)=>`<text x="30" y="${18+w/3*174+3}" font-size="9" fill="rgba(10,10,10,0.4)" text-anchor="end">\u20AC${(y/1e3).toFixed(0)}K</text>`).join(""),O=e>=0?`
    <line x1="${g(e)}" y1="18" x2="${g(e)}" y2="192" stroke="${a?"#019273":"#ef4444"}" stroke-width="1.5" stroke-dasharray="3 3" opacity="0.5"/>
    <circle cx="${g(e)}" cy="${m(s[e])}" r="6" fill="${a?"#019273":"#ef4444"}"/>
    <circle cx="${g(e)}" cy="${m(s[e])}" r="11" fill="none" stroke="${a?"#019273":"#ef4444"}" stroke-width="1.5" opacity="0.4"/>
    <text x="${g(e)}" y="${m(s[e])-16}" font-size="10" font-weight="500" fill="${a?"#019273":"#ef4444"}" text-anchor="middle">\u20AC${(s[e]/1e3).toFixed(0)}K</text>
  `:"";return`
    <svg viewBox="0 0 720 220" preserveAspectRatio="xMidYMid meet" style="width: 100%; height: auto; display: block;">
      ${k}
      ${_}
      <path d="${F}" fill="rgba(78,85,225,0.10)"/>
      <path d="${f}" fill="none" stroke="#0a0a0a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
      <path d="${j}" fill="none" stroke="#4e55e1" stroke-width="2" stroke-dasharray="4 3" stroke-linecap="round" stroke-linejoin="round"/>
      ${O}
      ${K}
    </svg>
  `}var z=[820,850,870,890,910,880,740,820,900,940,980,1010,1040].map(s=>s*1e3),Ie=[820,850,870,890,910,920,935,950,980,1e3,1030,1060,1090].map(s=>s*1e3),Pe={fx:{id:"fx",label:"EUR +5% vs USD",icon:"globe",title:"EUR strengthens 5% vs USD",sub:"Affects US revenue conversion \xB7 Q ends in week 13",intensities:["+2%","+5%","+10%"],defaultIntensity:"+5%",driverCopy:"US receivables book: $1.2M \xB7 locked at \u20AC0.92 \xB7 adjustment applied from week 4",weeks:(s="+5%")=>{let e=s==="+2%"?.972:s==="+10%"?.88:.93;return z.map((a,t)=>t<3?a:Math.round(a*e))},risk:(s="+5%")=>({w13:s==="+2%"?"\u20AC1.01M":s==="+10%"?"\u20AC916K":"\u20AC970K",delta:s==="+2%"?"\u2212\u20AC30K":s==="+10%"?"\u2212\u20AC124K":"\u2212\u20AC70K",exposure:s==="+2%"?"\u20AC35K":s==="+10%"?"\u20AC184K":"\u20AC92K",exposureDesc:"USD receivables"})},costs:{id:"costs",label:"Input costs +8%",icon:"fuel",title:"Input costs rise 8%",sub:"Hits margin on AP payables \xB7 customers pass less-than-full through",intensities:["+4%","+8%","+15%"],defaultIntensity:"+8%",driverCopy:"Raw mat index +8% WoW \xB7 AP book \u20AC2.4M across 6 suppliers \xB7 60% pass-through",weeks:(s="+8%")=>{let e=s==="+4%"?.975:s==="+15%"?.91:.955;return z.map((a,t)=>t<3?a:Math.round(a*e))},risk:(s="+8%")=>({w13:s==="+4%"?"\u20AC1.01M":s==="+15%"?"\u20AC946K":"\u20AC993K",delta:s==="+4%"?"\u2212\u20AC30K":s==="+15%"?"\u2212\u20AC94K":"\u2212\u20AC47K",exposure:s==="+4%"?"\u20AC28K":s==="+15%"?"\u20AC95K":"\u20AC58K",exposureDesc:"margin compression"})},dso:{id:"dso",label:"DSO +5 days",icon:"trendingUp",title:"DSO slips +5 days",sub:"Receipts shift right \xB7 same cash, later week",intensities:["+3d","+5d","+10d"],defaultIntensity:"+5d",driverCopy:"Customer AP teams taking longer \xB7 4 accounts showing drift \xB7 early warning",weeks:(s="+5d")=>{let e=s==="+3d"?.4:s==="+10d"?1.4:.7;return z.map((a,t)=>{if(t<3)return a;let o=Math.max(0,Math.min(z.length-1,t-e)),i=Math.floor(o),c=Math.ceil(o),p=o-i;return Math.round(z[i]*(1-p)+z[c]*p)})},risk:(s="+5d")=>({w13:s==="+3d"?"\u20AC1.02M":s==="+10d"?"\u20AC950K":"\u20AC985K",delta:s==="+3d"?"\u2212\u20AC20K":s==="+10d"?"\u2212\u20AC90K":"\u2212\u20AC55K",exposure:s==="+3d"?"\u20AC52K":s==="+10d"?"\u20AC178K":"\u20AC108K",exposureDesc:"cash timing"})},delay:{id:"delay",label:"Top customer 30d delay",icon:"truck",title:`${n.energy.name} delays 30 days`,sub:"\u20AC179K PTP at risk \xB7 historical slippage pattern applies",intensities:["15d","30d","60d"],defaultIntensity:"30d",driverCopy:`${n.energy.name} \xB7 \u20AC179K invoice \xB7 88% slip risk \xB7 2 broken PTPs in 60d`,weeks:(s="30d")=>{let e=s==="15d"?110:179,a=s==="15d"?8:s==="60d"?12:10;return z.map((t,o)=>o===6?t-e*1e3:o===a&&s!=="60d"?t+Math.round(e*.8*1e3):t)},risk:(s="30d")=>({w13:s==="15d"?"\u20AC1.02M":s==="60d"?"\u20AC861K":"\u20AC931K",delta:s==="15d"?"\u2212\u20AC20K":s==="60d"?"\u2212\u20AC179K":"\u2212\u20AC109K",exposure:s==="15d"?"\u20AC110K":"\u20AC179K",exposureDesc:`${n.energy.name} PTP`})}};function Ve(s,e){let h=z.length,g=k=>36+k/(h-1)*668,b=[...z,...s],u=Math.max(...b)*1.05,m=Math.min(...b)*.9,l=k=>14+(1-(k-m)/(u-m))*150,f=k=>k.map((_,O)=>`${O===0?"M":"L"}${g(O).toFixed(1)},${l(_).toFixed(1)}`).join(" "),j=[0,.33,.66,1].map(k=>{let _=14+k*150;return`<line x1="36" y1="${_}" x2="704" y2="${_}" stroke="rgba(10,10,10,0.05)" stroke-width="1"/>`}).join(""),V=[0,3,6,9,12].map(k=>`<text x="${g(k)}" y="182" font-size="9" fill="rgba(10,10,10,0.45)" text-anchor="middle">W${k+1}</text>`).join(""),W=[u,(u+m)/2,m].map((k,_)=>`<text x="30" y="${14+_/2*150+3}" font-size="9" fill="rgba(10,10,10,0.4)" text-anchor="end">\u20AC${(k/1e3).toFixed(0)}K</text>`).join(""),F=g(h-1),K=l(s[h-1]);return`
    <svg viewBox="0 0 720 190" preserveAspectRatio="xMidYMid meet" style="width: 100%; height: auto; display: block;">
      ${j}
      ${W}
      <path d="${f(z)}" fill="none" stroke="rgba(10,10,10,0.25)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
      <path d="${f(s)}" fill="none" stroke="#4e55e1" stroke-width="2" stroke-dasharray="4 3" stroke-linecap="round" stroke-linejoin="round"/>
      <circle cx="${F}" cy="${K}" r="4" fill="#4e55e1"/>
      <text x="${F-6}" y="${K-8}" font-size="10" font-weight="500" fill="#4e55e1" text-anchor="end">${e}</text>
      ${V}
    </svg>
  `}function J(s,e){let a=Pe[s],t=e||a.defaultIntensity,o=a.weeks(t),i=a.risk(t);return`
    <div class="sc-head">
      <div>
  ...
  `}var Oe={id:"predictions",tag:"CASH FLOW FORECASTING \xB7 INTERACTIVE DEMO",coverLabel:"See cash predictions in 30 seconds",closing:{headline:'A \u20AC170K dip avoided. Forecast back on track. <span class="grad">Hours back.</span>',sub:"Want Vero to monitor and act on signals every morning?"},scenes:[{id:"p-01-forecast",title:"Step 1 \xB7 13-week forecast",body:'Cash holding strong, except a <span class="grad">\u20AC170K dip</span> in week 6. Vero spotted the risk before it lands.',tooltipSide:"left",spotlight:"#dip-investigate",advanceOn:{click:"#dip-investigate"},html:`${R}
      ...
      `},{id:"p-02-scenario",title:"Step 2 \xB7 Stress-test",body:'<span class="grad">Click any scenario</span>. The chart and numbers update live. Try switching and dialling intensities up.',tooltipSide:"top",spotlight:"#scenarios-frame",advanceOn:{click:"#scenario-drilldown"},onMount:s=>{let e=s.querySelector("#scenario-inner");e&&(s.querySelectorAll("[data-scenario-tab]").forEach(a=>{a.addEventListener("click",()=>{let t=a.getAttribute("data-scenario-tab");s.querySelectorAll("[data-scenario-tab]").forEach(o=>o.classList.toggle("active",o===a)),e.innerHTML=J(t)})}),s.addEventListener("click",a=>{let t=a.target.closest("[data-intensity]");if(!t)return;let o=t.getAttribute("data-scenario"),i=t.getAttribute("data-intensity");e.innerHTML=J(o,i)}))},html:`${R}
        ...
        <div id="scenarios-frame">
          ...
          <div id="scenario-inner">${J("fx")}</div>
          <div class="cta-bar">
            <div>
      ...
      `},{id:"p-03-atrisk",title:"Step 3 \xB7 At-risk invoices",body:`${n.energy.name}'s \u20AC179K is the biggest exposure. <span class="grad">88% likely to slip</span> based on past behaviour plus signals.`,tooltipSide:"bottom",spotlight:"#energy-row",advanceOn:{click:"#energy-row"},html:`${R}
        <style>
        ...
        <div class="card">
          ...
          ${Le.map((s,e)=>{let a=e===0,t=s.risk>=80?"red":s.risk>=65?"amber":"gray",o=s.risk>=80?"#ef4444":s.risk>=65?"#f59e0b":"#94a3b8";return`
      ...
      `},{id:"p-04-recommend",title:"Step 4 \xB7 Vero's plan",body:`Three risk signals stack up. Vero's plan: <span class="grad">act now</span>, before the dip lands.`,tooltipSide:"left",spotlight:"#pred-apply",advanceOn:{click:"#pred-apply"},html:`${R}
        <style>
      ...
      `},{id:"p-05-resolved",title:"Step 5 \xB7 Forecast restored",body:'Dip mitigated. Forecast back on trend. Want Vero to <span class="grad">monitor and act</span> on these signals every morning?',tooltipSide:"top",spotlight:"#pred-yes",advanceOn:{click:"#pred-yes"},html:`${R}
        <style>
        ...
        <div class="chart-card">
          ...
          ${re({weeks:Ie,highlight:6,resolved:!0})}
        </div>
...
`,Be={id:"vero-chat",tag:"VERO CHAT \xB7 INTERACTIVE DEMO",coverLabel:"See Vero Chat in 30 seconds",closing:{headline:'Ask anything. Get answers and <span class="grad">actions</span>.',sub:"Want Vero to monitor your risk landscape and brief you every morning?"},scenes:[{id:"v-01-ask",title:"Step 1 \xB7 Ask",body:'Type any question. Vero pulls from your <span class="grad">live data</span>. No SQL, no joins, no waiting on BI.',tooltipSide:"left",spotlight:"#send-btn",advanceOn:{click:"#send-btn"},html:`${G}
      ...
      `},{id:"v-02-thinking",title:"Step 2 \xB7 Thinking",body:'Vero calls the right tools (aging report, credit scoring, communications history) <span class="grad">in parallel</span>.',tooltipSide:"right",spotlight:"#tool-stack",html:`${G}
        <style>
      ...
      `},{id:"v-03-answer",title:"Step 3 \xB7 Answer",body:'Ranked, scored, with the why. The action chips are <span class="grad">one click</span>. Not just an answer, an offer to act.',tooltipSide:"top",spotlight:"#schedule-calls-btn",advanceOn:{click:"#schedule-calls-btn"},html:`${G}
        <style>
              ...
              <div class="msg-vero-content">
                <div class="msg-vero-bubble">
                  ...
                  <div class="ranked-list">
                    ${Re.slice(0,3).map((s,e)=>{let a=s.risk>=80?"r-red":s.risk>=65?"r-amber":"r-gray";return`
      ...
      `},{id:"v-04-done",title:"Step 4 \xB7 Done",body:'Three calls scheduled in <span class="grad">one click</span>. Want Vero to flag risks and brief you every morning?',tooltipSide:"top",spotlight:"#chat-yes",advanceOn:{click:"#chat-yes"},html:`${G}
        <style>
...
`,De={id:"vero-chat-v2",tag:"VERO AGENT \xB7 INTERACTIVE DEMO",coverLabel:"Meet Vero, your AR team lead",closing:{headline:'Reviewed, approved, fired. <span class="grad">One agent</span> running your AR.',sub:"This is what your mornings could look like."},scenes:[{id:"vb-01-recap",title:"Step 1 \xB7 Overnight recap",body:`Vero already handled the obvious work. Below is what's left, <span class="grad">waiting on you</span>.`,tooltipSide:"top",spotlight:"#vb-start-review",advanceOn:{click:"#vb-start-review"},html:`${B}
      ...
      `},{id:"vb-02-jes",title:"Step 2 \xB7 Cash app \xB7 2 JEs",body:'Every ledger line is visible. No black box, <span class="grad">no surprises</span>.',tooltipSide:"top",spotlight:"#vb-approve-jes",advanceOn:{click:"#vb-approve-jes"},html:`${B}
        <style>
      ...
      `},{id:"vb-03-calls",title:"Step 3 \xB7 Collections \xB7 3 calls",body:'Per-customer script, language and time. Vero waits for your <span class="grad">green light</span>.',tooltipSide:"top",spotlight:"#vb-approve-calls",advanceOn:{click:"#vb-approve-calls"},html:`${B}
        <style>
      ...
      `},{id:"vb-04-flag",title:"Step 4 \xB7 Deductions \xB7 1 flag",body:'Vero settled 5 itself. This one hit the <span class="grad">guardrail</span>, packaged for you.',tooltipSide:"top",spotlight:"#vb-send-to-queue",advanceOn:{click:"#vb-send-to-queue"},html:`${B}
        <style>
      ...
      `},{id:"vb-05-schedule",title:"Step 5 \xB7 Schedule as a task",body:'Six items in three clicks. Let Vero run this morning <span class="grad">every morning</span>.',tooltipSide:"top",spotlight:"#vb-schedule",advanceOn:{click:"#vb-schedule"},html:`${B}
        <style>
...
`;var le=[{main:"Your AR, on autopilot.",sub:"From chaos to clarity in seconds."},{main:"One agent. Every workflow.",sub:"Collections, payments, deductions, forecasts."},{main:"You ask. Vero delivers.",sub:"The AI finance agent that gets things done."}],Ne=["\u20AC890K cleared, 3 JEs posted","PTP \u20AC179K confirmed for 25 Apr","\u20AC3,780 valid trade promo, \u20AC450 to review","\u20AC14.2M projected, +6% from new PTP"],qe=["Ask","Ingest","Analyze","Execute","Complete","Review","Report"],Fe=[3,7,5,9,4,8,6,10,3,7,5,8,4,6,9,5,7,3],de=[{x1:195,y1:55,x2:370,y2:215},{x1:185,y1:170,x2:370,y2:235},{x1:190,y1:305,x2:370,y2:260},{x1:185,y1:420,x2:370,y2:280}],pe=[{x1:530,y1:215,x2:660,y2:50},{x1:530,y1:235,x2:660,y2:165},{x1:530,y1:260,x2:660,y2:300},{x1:530,y1:280,x2:660,y2:405}],ve={x1:780,y1:225,x2:730,y2:400},Ke=[{p:0,d:0},{p:1,d:2200},{p:2,d:4e3},{p:3,d:5800},{p:4,d:7800},{p:5,d:9800},{p:6,d:13e3},{p:7,d:16500},{p:-1,d:2e4}],He=`
...
`,X=class extends HTMLElement{constructor(){super(),this._phase=-1,this._cycle=0,this._timers=[],this._avatarTimers=[],this._started=!1,this._paused=!1,this._observer=null}connectedCallback(){if(this._shadow)return;this._shadow=this.attachShadow({mode:"open"}),this._shadow.innerHTML=`<style>${ce}</style>${He}`,this._q=t=>this._shadow.querySelector(t),this._buildStaticBits();let e=()=>{this._started||(this._started=!0,this._runCycle())},a=()=>{"requestIdleCallback"in window?window.requestIdleCallback(e,{timeout:500}):setTimeout(e,200)};"IntersectionObserver"in window?(this._observer=new IntersectionObserver(t=>{t[0].isIntersecting?(this._paused=!1,this._started||a()):this._paused=!0},{threshold:.01}),this._observer.observe(this)):a()}disconnectedCallback(){this._timers.forEach(clearTimeout),this._timers=[],this._clearAvatarTimers(),this._observer?.disconnect()}_buildStaticBits(){let e=this._q("#waveform");Fe.forEach((i,c)=>{let p=document.createElement("div");p.className="waveform-bar",p.setAttribute("data-h",i),p.style.transitionDelay=c*30+"ms",e.appendChild(p)});let a=this._q("#progress-steps");qe.forEach((i,c)=>{let p=document.createElement("div");p.className="progress-step",p.innerHTML=`<div class="progress-dot" data-step="${c}"></div><span class="progress-label" data-step="${c}">${i}</span>`,a.appendChild(p)});let t=i=>{let c=(i.x1+i.x2)/2,p=(i.x1+i.x2)/2;return`M${i.x1},${i.y1} C${c},${i.y1} ${p},${i.y2} ${i.x2},${i.y2}`},o=i=>Math.sqrt(Math.pow(i.x2-i.x1,2)+Math.pow(i.y2-i.y1,2))*1.5;this._allConns=[];for(let i=0;i<4;i++){let c=this._q(`#conn-in-${i}`),p=t(de[i]),d=o(de[i]);c.setAttribute("d",p),c.style.strokeDasharray=d,c.style.strokeDashoffset=d,c.style.transition=`stroke-dashoffset 0.8s cubic-bezier(0.22,1,0.36,1) ${i*.12}s`,this._allConns.push({el:c,len:d,phase:2})}for(let i=0;i<4;i++){let c=this._q(`#conn-out-${i}`),p=t(pe[i]),d=o(pe[i]);c.setAttribute("d",p),c.style.strokeDasharray=d,c.style.strokeDashoffset=d,c.style.transition=`stroke-dashoffset 0.8s cubic-bezier(0.22,1,0.36,1) ${i*.12}s`,this._allConns.push({el:c,len:d,phase:3})}{let i=this._q("#conn-cross"),c=t(ve),p=o(ve);i.setAttribute("d",c),i.style.strokeDasharray=p,i.style.strokeDashoffset=p,i.style.transition="stroke-dashoffset 0.8s cubic-bezier(0.22,1,0.36,1) 0.3s",this._allConns.push({el:i,len:p,phase:4})}this._drawMiniChart(!1)}_drawMiniChart(e){let a=[6,5.5,6.2,5.8,6.5,7,e?7.8:7.2],t=130,o=40,i=4,c=8.5,p=4.5,d=u=>i+u/(a.length-1)*(t-2*i),v=u=>i+(c-u)/(c-p)*(o-2*i),h=a.map((u,m)=>(m===0?"M":"L")+d(m)+","+v(u)).join(" "),g=this._q("#mini-chart"),b=`<path d="${h}" fill="none" stroke="${e?"#6366f1":"#d1d5db"}" stroke-width="2" stroke-linecap="round" style="transition:all 0.8s ease"/>`;e&&(b+=`<path d="M${d(5)},${v(7)} L${d(6)},${v(7.8)} L${d(6)},${v(7.2)} L${d(5)},${v(7)} Z" fill="#818cf8" opacity="0.2"/>`,b+=`<circle cx="${d(6)}" cy="${v(7.8)}" r="3" fill="#6366f1"/>`,b+=`<line x1="${d(5)}" y1="${v(7.2)}" x2="${d(6)}" y2="${v(7.2)}" stroke="#d1d5db" stroke-width="1" stroke-dasharray="3 2"/>`),g.innerHTML=b}_clearAvatarTimers(){this._avatarTimers.forEach(e=>{clearTimeout(e),clearInterval(e)}),this._avatarTimers=[]}_animateVeroAvatar(e,a){if(!e)return;let t=e.querySelector(".vero-avatar-text");if(!t)return;let o=t.querySelectorAll("span");if(a==="idle"){t.style.color="#10B981",o[0].textContent="0",o[1].textContent="0";let i=()=>{o[0].textContent="-",this._avatarTimers.push(setTimeout(()=>{o[0].textContent="0"},150)),this._avatarTimers.push(setTimeout(i,700+Math.random()*2e3))};this._avatarTimers.push(setTimeout(i,700+Math.random()*1e3));let c=()=>{o[1].textContent="-",this._avatarTimers.push(setTimeout(()=>{o[1].textContent="0"},150)),this._avatarTimers.push(setTimeout(c,1e3+Math.random()*2500))};this._avatarTimers.push(setTimeout(c,1e3+Math.random()*1500))}else if(a==="thinking"){t.style.color="#6EE7B7";let i=setInterval(()=>{o[0].textContent=Math.random()>.5?"1":"0",o[1].textContent=Math.random()>.5?"1":"0"},80);this._avatarTimers.push(i)}else a==="done"&&(t.style.color="#10B981",o[0].textContent="^",o[1].textContent="^")}_setPhase(e){this._phase=e,this._clearAvatarTimers();let a=e===0||e===6,t=e>=1&&e<=4,o=e===5,i=e===7;this._q("#chat-overlay").classList.toggle("visible",a);let p=this._q("#chat-human");p.className="chat-human"+(e===0?" visible":e===6?" faded":""),p.style.transitionDelay=e===0?"0.3s":"0s";let d=this._q("#chat-thinking");d.className="chat-vero-thinking"+(e===0?" visible":""),d.style.transitionDelay=e===0?"1s":"0s";let v=this._q("#chat-summary");v.className="chat-vero-summary"+(e===6?" visible":""),v.style.transitionDelay=e===6?"0.5s":"0s",e===0&&this._animateVeroAvatar(this._q("#vero-avatar-chat-thinking"),"thinking"),e===6&&this._animateVeroAvatar(this._q("#vero-avatar-chat-summary"),"done"),this._q("#canvas-layer").classList.toggle("visible",t);for(let l=0;l<4;l++)this._q("#input-"+l).classList.toggle("visible",e>=1);this._allConns.forEach(l=>{l.el.style.strokeDashoffset=e>=l.phase?"0":l.len});let h=this._q("#vero-hub");h.classList.toggle("visible",e>=2),h.classList.toggle("pulsing",e>=2&&e<=4),this._q("#vero-hub-card").classList.toggle("active",e>=3);let g=this._q("#vero-hub-status");g.textContent=e<3?"Analyzing...":"Running 4 workflows",g.style.color=e>=3?"var(--violet-600)":"var(--gray-400)",e>=2&&this._animateVeroAvatar(this._q("#vero-avatar-hub"),e>=3?"thinking":"idle");for(let l=0;l<4;l++){let f=this._q("#output-"+l);f.classList.toggle("visible",e>=3),f.classList.toggle("done",e>=4),this._q("#check-"+l).classList.toggle("visible",e>=4)}for(let l=0;l<3;l++)this._q("#inv-"+l).classList.toggle("done",e>=4),this._q("#badge-"+l).classList.toggle("done",e>=4);this._q("#cleared-text").classList.toggle("visible",e>=4),this._q("#waveform").querySelectorAll(".waveform-bar").forEach(l=>{let f=e>=3;l.classList.toggle("active",f);let j=parseFloat(l.getAttribute("data-h")),V=f?Math.min(1,j*2.2/22):.14;l.style.transform=`scaleY(${V})`}),this._q("#call-detail").classList.toggle("visible",e>=4);for(let l=0;l<3;l++){let f=this._q("#ev-check-"+l);f.classList.toggle("done",e>=4),f.classList.toggle("pending",e<4),f.style.transitionDelay=l*150+400+"ms"}if(this._q("#verdict-fill").classList.toggle("done",e>=4),this._drawMiniChart(e>=4),this._q("#forecast-uplift").classList.toggle("visible",e>=4),this._q("#checklist-overlay").classList.toggle("visible",o),o){this._animateVeroAvatar(this._q("#vero-avatar-checklist"),"thinking"),this._resetTodos();for(let l=0;l<4;l++)(f=>{this._timers.push(setTimeout(()=>this._checkTodo(f),600+f*500))})(l)}if(this._q("#tagline-overlay").classList.toggle("visible",i),i){let l=le[this._cycle%le.length];this._q("#tagline-content").innerHTML=`<div class="tagline-main">${l.main}</div><div class="tagline-sub">${l.sub}</div>`}let m=Math.min(100,e/7*100);this._q("#progress-fill").style.width=m+"%",this._q("#progress-steps").querySelectorAll(".progress-dot").forEach(l=>{let f=parseInt(l.getAttribute("data-step"));l.className="progress-dot"+(e>=f?f<=1?" active-v":f<=4?" active-e":" active-b":"")}),this._q("#progress-steps").querySelectorAll(".progress-label").forEach(l=>{let f=parseInt(l.getAttribute("data-step"));l.classList.toggle("active",e>=f)})}_resetTodos(){for(let e=0;e<4;e++)this._q("#todo-box-"+e).className="check-box",this._q("#todo-box-"+e).innerHTML="",this._q("#todo-label-"+e).className="check-label",this._q("#todo-sub-"+e).className="check-sub",this._q("#todo-sub-"+e).textContent="Processing...";this._q("#checklist-footer").classList.remove("visible")}_checkTodo(e){let a=this._q("#todo-box-"+e);a.className="check-box checked",a.innerHTML='<svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M2.5 6L5 8.5L9.5 3.5" stroke="white" stroke-width="1.5" stroke-linecap="round"/></svg>',this._q("#todo-label-"+e).classList.add("checked");let t=this._q("#todo-sub-"+e);t.classList.add("checked"),t.textContent=Ne[e],e===3&&this._q("#checklist-footer").classList.add("visible")}_runCycle(){this._timers.forEach(clearTimeout),this._timers=[],Ke.forEach(e=>{this._timers.push(setTimeout(()=>{this._paused||(e.p===-1?(this._cycle++,this._runCycle()):this._setPhase(e.p))},e.d))})}};customElements.get("transformance-hero")||customElements.define("transformance-hero",X);var ge=`
:host{
  --tf-canvas:#f6f4f1;
  --tf-dark:#000;
  --tf-ink:#000;
...
`;var I={"cash-forecast":{headline:"Forecast cash 90 days out at 95% accuracy. Across AR and AP.",body:"Multi-horizon models. Confidence ranges, not point estimates. FX-aware.",ctaPrimary:"Book a 15-min call",ctaPrimaryUrl:"/meeting",ctaSecondary:"Visit CashPulse \u2192",ctaSecondaryUrl:"/solutions/cash-flow-forecasting"},"cash-app":{headline:"Match 95% of payments straight-through. No templates.",body:"Vision LLMs read any remittance format. >95% extraction accuracy. Live in 4 to 8 weeks.",ctaPrimary:"Book a 15-min call",ctaPrimaryUrl:"/meeting",ctaSecondary:"Visit ClearMatch \u2192",ctaSecondaryUrl:"/solutions/cash-application"},collections:{headline:"Cut DSO by 8 to 15 days with autonomous AI collectors.",body:"30+ languages. 15 to 20 calls per hour per agent versus 15 to 20 per day for a human.",ctaPrimary:"Book a 15-min call",ctaPrimaryUrl:"/meeting",ctaSecondary:"Visit CollectPulse \u2192",ctaSecondaryUrl:"/solutions/collections"},deductions:{headline:"Resolve deductions 6\xD7 faster at 97% accuracy.",body:"Six-category auto-classification. Graph-based investigation. Audit trail built in.",ctaPrimary:"Book a 15-min call",ctaPrimaryUrl:"/meeting",ctaSecondary:"Visit ClaimIQ \u2192",ctaSecondaryUrl:"/solutions/deductions"},vero:{headline:"One AI agent across cash app, deductions, collections, and forecasting.",body:"Persistent memory. Learns your ops over time. Deploys inside your cloud.",ctaPrimary:"Book a 15-min call",ctaPrimaryUrl:"/meeting",ctaSecondary:"Meet Vero \u2192",ctaSecondaryUrl:"/solutions/vero-agent"},o2c:{headline:"Run the full O2C cycle on one AI-native platform.",body:"Cash application, collections, deductions, and forecasting. Unified by Vero. Deploys in 4 to 8 weeks.",ctaPrimary:"Book a 15-min call",ctaPrimaryUrl:"/meeting",ctaSecondary:"See Solutions \u2192",ctaSecondaryUrl:"/solutions"},"vendor-comparison":{headline:"An AI-native alternative built for execution, not reporting.",body:"Most AR suites surface insights. Transformance acts on them. See the difference in 4 to 8 weeks.",ctaPrimary:"Book a 15-min call",ctaPrimaryUrl:"/meeting",ctaSecondary:"See Solutions \u2192",ctaSecondaryUrl:"/solutions"},default:{headline:"Fix your AR data. The forecast fixes itself.",body:"AI-native O2C automation. Cash app, deductions, collections, forecasting. One agent, four products.",ctaPrimary:"Book a 15-min call",ctaPrimaryUrl:"/meeting",ctaSecondary:"See Solutions \u2192",ctaSecondaryUrl:"/solutions"}},P={name:"Paul, founder",role:"Transformance",imageUrl:"https://cdn.prod.website-files.com/684931abb239b84984296d93/68494720f0211b2372893faf_Profile%20Picture.avif",imageAlt:"Paul Hanke, Co-Founder of Transformance"};function we(s){return I[s]||I.default}var ee=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get observedAttributes(){return["data-variant","data-state","data-suppress-secondary"]}attributeChangedCallback(){this.shadowRoot.firstChild&&this._render()}connectedCallback(){this.hasAttribute("data-state")||this.setAttribute("data-state","hidden"),this._render()}show(){requestAnimationFrame(()=>{this.setAttribute("data-state","visible")})}hide(){this.setAttribute("data-state","hidden")}_render(){let e=this.getAttribute("data-variant")||"default",a=we(e),t=this.getAttribute("data-suppress-secondary")==="true",o=`
      <style>${ke}</style>
      <div class="card" role="dialog" aria-label="Talk to Transformance">
        ...
        <div class="author">
          <div class="avatar">${P.imageUrl?`<img src="${S(P.imageUrl)}" alt="${S(P.imageAlt||"")}">`:"<span>PH</span>"}</div>
          <div class="author-meta">
            <span class="author-name">${S(P.name)}</span>
            <span class="author-role">${S(P.role)}</span>
          </div>
        </div>
        <h4 class="headline">${S(a.headline)}</h4>
        <p class="body">${S(a.body)}</p>
        <a class="cta-primary" href="${S(a.ctaPrimaryUrl||"/meeting")}" data-action="cta-primary">${S(a.ctaPrimary)}</a>
        <a class="cta-secondary" href="${S(a.ctaSecondaryUrl||"/solutions")}" data-action="cta-secondary"${t?" hidden":""}>${S(a.ctaSecondary)}</a>
      </div>
    `;this.shadowRoot.innerHTML=o,this.shadowRoot.querySelector('[data-action="dismiss"]').addEventListener("click",i=>{i.preventDefault(),this.dispatchEvent(new CustomEvent("tf-popup-dismiss",{bubbles:!0,composed:!0}))}),this.shadowRoot.querySelector('[data-action="cta-primary"]').addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("tf-popup-cta",{bubbles:!0,composed:!0,detail:{kind:"primary",href:a.ctaPrimaryUrl}}))}),this.shadowRoot.querySelector('[data-action="cta-secondary"]').addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("tf-popup-cta",{bubbles:!0,composed:!0,detail:{kind:"secondary",href:a.ctaSecondaryUrl}}))})}};function S(s){return String(s??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}customElements.get("transformance-popup")||customElements.define("transformance-popup",ee);var Se="tf_popup_dismissed_",We=30*24*60*60*1e3,Ye=35,Qe=25*1e3,Je={"/solutions/cash-flow-forecasting":"cash-forecast","/solutions/cash-application":"cash-app","/solutions/collections":"collections","/solutions/claims-and-deductions":"deductions","/solutions/deductions":"deductions","/solutions/vero-agent":"vero","/solutions":"o2c"},$e={"cash-forecasting":"cash-forecast","cash-flow-forecasting":"cash-forecast","cash-flow":"cash-forecast",forecasting:"cash-forecast","cash-application":"cash-app","cash-app":"cash-app",collections:"collections",collection:"collections",deductions:"deductions",deduction:"deductions",claims:"deductions","claims-and-deductions":"deductions",vero:"vero",o2c:"o2c","order-to-cash":"o2c","order-to-cash-broad":"o2c","invoice-to-cash":"o2c","ar-automation":"o2c","vendor-comparison":"vendor-comparison","vendor-comparisons":"vendor-comparison",comparisons:"vendor-comparison",competitors:"vendor-comparison"};function Xe(s){if(!s)return null;let e=String(s).trim().toLowerCase().replace(/\s+/g,"-");return I[e]?e:$e[e]?$e[e]:null}function Ze(s){return s?/^ar-cash-forecasting|^13-week|cash-flow-forecast|automated-cash-flow|why-are-most-cash-flow/.test(s)?"cash-forecast":/cash-application|cash-app|payment-reconciliation|auto-cash/.test(s)?"cash-app":/collections|dunning|reducing-dso|best-tools-for-reducing-dso/.test(s)?"collections":/deduction|claim|partial-payments/.test(s)?"deductions":/highradius|billtrust|blackline|competitors|alternatives|kyriba|tesorio/.test(s)?"vendor-comparison":/order-to-cash|o2c|invoice-to-cash|accounts-receivable-automation|what-is-order/.test(s)?"o2c":"default":"default"}function es(){let s=window.location.pathname.replace(/\/+$/,"");for(let[e,a]of Object.entries(Je))if(s===e)return{variant:a,kind:"solution",slug:e};if(s.startsWith("/blog-posts/")){let e=document.querySelector('meta[name="tf-blog-category"]'),a=Xe(e&&e.getAttribute("content")),t=s.split("/").pop()||"";return{variant:a||Ze(t),kind:"blog",slug:t}}return null}function ss(s){try{let e=localStorage.getItem(Se+s);if(!e)return!1;let a=Number(e);return Number.isFinite(a)&&Date.now()-a<We}catch{return!1}}function as(s){try{localStorage.setItem(Se+s,String(Date.now()))}catch{}}function se(s,e){try{window.dataLayer=window.dataLayer||[],window.dataLayer.push({event:s,...e})}catch{}}function ts(){return document.querySelector("article")||document.querySelector('[role="main"]')||document.querySelector("main")||document.body}function is(s){let e=s.getBoundingClientRect(),a=s===document.body?document.documentElement.scrollHeight:s.scrollHeight,t=window.innerHeight,o=window.scrollY-(s.offsetTop||0);return a<=t?100:(o+t)/a*100}var ze=!1,ns=(()=>{try{return/[?&]tf_debug=1/.test(window.location.search)||localStorage.getItem("tf_popup_debug")==="1"}catch{return!1}})();function N(...s){ns&&console.log("[tf-popup]",...s)}function ae(){if(ze)return;ze=!0,N("installPopup() called",{path:window.location.pathname});let s=es();if(!s){N("no ctx \u2014 exiting (not a blog or solution page)");return}if(N("resolved ctx",s),ss(s.variant)){N("dismissed within 30 days \u2014 exiting",{key:"tf_popup_dismissed_"+s.variant});return}let e=I[s.variant]||I.default,a=e&&e.ctaSecondaryUrl&&window.location.pathname.replace(/\/+$/,"")===e.ctaSecondaryUrl.replace(/\/+$/,""),t=document.createElement("transformance-popup");t.setAttribute("data-variant",s.variant),t.setAttribute("data-state","hidden"),a&&t.setAttribute("data-suppress-secondary","true"),document.body.appendChild(t);let o=!1,i=0,c,p,d=b=>{o||(o=!0,i=Date.now(),v(),t.show(),N("popup shown",{source:b,variant:s.variant}),se("popup_shown",{variant:s.variant,slug:s.slug,kind:s.kind,trigger:b}))},v=()=>{c&&(window.removeEventListener("scroll",c,{passive:!0}),c=null),p&&(clearTimeout(p),p=null)},h=ts(),g=0;c=()=>{let b=Date.now();b-g<100||(g=b,is(h)>=Ye&&d("scroll"))},window.addEventListener("scroll",c,{passive:!0}),p=setTimeout(()=>d("timer"),Qe),t.addEventListener("tf-popup-dismiss",()=>{t.hide(),as(s.variant),se("popup_dismissed",{variant:s.variant,slug:s.slug,time_visible_ms:Date.now()-i}),setTimeout(()=>t.remove(),400)}),t.addEventListener("tf-popup-cta",b=>{se("popup_cta_clicked",{variant:s.variant,slug:s.slug,cta:b.detail.kind,target_url:b.detail.href})})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ae,{once:!0}):ae();var os=[{name:"Pre-Due Reminder",day:"Day -3",subject:"Friendly reminder: Invoice [INV-####] due [Due Date]",body:`Hi [First Name],
...
Thank you,
[Your Name]
[Company] | Accounts Receivable`},{name:"First Reminder",day:"Day 1-3 Past Due",subject:"Invoice [INV-####] is now past due",body:`Hi [First Name],
...
Thank you,
[Your Name]
[Company] | Accounts Receivable`},{name:"Second Notice",day:"Day 8-14 Past Due",subject:"Second notice: Invoice [INV-####] outstanding [X] days",body:`Hi [First Name],
...
[Your Name]
[Company] | Accounts Receivable`},{name:"Firm Notice",day:"Day 15-30 Past Due",subject:"Urgent: Invoice [INV-####] requires immediate attention",body:`Dear [First Name],
...
[Direct contact]`},{name:"Escalation Notice",day:"Day 31-45 Past Due",subject:"Final notice before escalation: Invoice [INV-####]",body:`Dear [First Name],
...
[Your Name]
[Company] | Accounts Receivable`},{name:"Final Demand",day:"Day 46+",subject:"Final demand: Invoice [INV-####], [Amount] outstanding",body:`Dear [First Name],
...
[Your Name]
[Company] | Accounts Receivable`}],rs={dunning:os},U={gmail:'<svg viewBox="0 49.4 512 399.42"><g fill="none" fill-rule="evenodd"><g fill-rule="nonzero"><path fill="#4285f4" d="M34.91 448.818h81.454V251L0 163.727V413.91c0 19.287 15.622 34.91 34.91 34.91z"/><path fill="#34a853" d="M395.636 448.818h81.455c19.287 0 34.909-15.622 34.909-34.909V163.727L395.636 251z"/><path fill="#fbbc04" d="M395.636 99.727V251L512 163.727v-46.545c0-43.142-49.25-67.782-83.782-41.891z"/></g><path fill="#ea4335" d="M116.364 251V99.727L256 204.455 395.636 99.727V251L256 355.727z"/><path fill="#c5221f" fill-rule="nonzero" d="M0 117.182v46.545L116.364 251V99.727L83.782 75.291C49.25 49.4 0 74.04 0 117.18z"/></g></svg>',outlook:'<svg viewBox="0 0 24 24"><rect x="2.5" y="4.5" width="19" height="15" rx="3.2" fill="#0F6CBD"/><circle cx="12" cy="12" r="4.6" fill="none" stroke="#fff" stroke-width="2.1"/></svg>',mail:'<svg viewBox="0 0 24 24"><rect x="2.5" y="5" width="19" height="14" rx="2.4" fill="none" stroke="#0EA5A5" stroke-width="1.9"/><path d="M3.5 7l8.5 6 8.5-6" fill="none" stroke="#0EA5A5" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/></svg>',dl:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v12m0 0l-4-4m4 4l4-4M4 19h16"/></svg>'},cs=`
...
`;function q(s){return String(s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function _e(s){return encodeURIComponent(s)}var te=class extends HTMLElement{connectedCallback(){this._m||(this._m=!0,this._render())}_render(){let e=rs[this.dataset.set||"dunning"];if(!e||!e.length){this.style.display="none";return}let a=this.dataset.docx||"",t=a?`<a class="dl" href="${q(a)}" download>${U.dl} Download all ${e.length} templates (.docx)</a>`:"",o=e.map((d,v)=>{let h=_e(d.subject),g=_e(d.body),b=`mailto:?subject=${h}&body=${g}`,u=`https://outlook.office.com/mail/deeplink/compose?subject=${h}&body=${g}`,m=`https://mail.google.com/mail/?view=cm&fs=1&su=${h}&body=${g}`;return`<div class="tmpl">
        <div class="head"><span class="bucket">${q(d.name)}</span><span class="day">${q(d.day)}</span></div>
        <div class="mail">
          <div class="mrow"><span class="ml">To</span><span class="mv">[First Name] &lt;customer@company.com&gt;</span></div>
          <div class="mrow subj"><span class="ml">Subject</span><span class="mv">${q(d.subject)}</span></div>
          <div class="body">${q(d.body)}</div>
        </div>
        <div class="acts">
          <button class="btn copy" data-i="${v}">Copy text</button>
          <a class="btn" target="_blank" rel="noopener" href="${b}">${U.mail}Mail</a>
          <a class="btn" target="_blank" rel="noopener" href="${u}">${U.outlook}Outlook</a>
          <a class="btn" target="_blank" rel="noopener" href="${m}">${U.gmail}Gmail</a>
        </div></div>`}).join(""),i=this.attachShadow({mode:"open"});i.innerHTML=`<style>${cs}</style><div class="wrap">${t}${o}${t}</div><div class="toast"></div>`;let c=i.querySelector(".toast"),p=d=>{c.textContent=d,c.classList.add("show"),setTimeout(()=>c.classList.remove("show"),1400)};i.querySelectorAll(".btn.copy").forEach(d=>{d.addEventListener("click",()=>{let v=e[+d.dataset.i];navigator.clipboard.writeText(v.subject+`

`+v.body).then(()=>{d.textContent="Copied",d.classList.add("done"),p("Copied to clipboard"),setTimeout(()=>{d.textContent="Copy text",d.classList.remove("done")},1600)})})})}};customElements.get("transformance-email-templates")||customElements.define("transformance-email-templates",te);})();
